  ...
  <div class="title">
    <div  style="background-color:#e0e0e0;" class="widget-header widget-header-small">
      <h6 class="widget-title smaller">
        <font size="3" color="blue"><b>Error Attendance Data</b></font>
      </h6>
    </div>
  </div>
  <div class="container">
    <form id='myform' action="{{route('get.attend.error.data')}}" id="acc_form" method="post">
      {{ csrf_field() }}
          ...
          <table class="table table-striped table-data table-report"> 
            <tbody>
              ...
              <tr>
                ...
                <td align="center">{{ $row->attDate }}</td>
                <td align="center">{{ date('H:m:s',strtotime($row->tInTime)) }}</td> 
                <td align="center">{{ date('H:m:s',strtotime($row->tOutTime)) }}</td> 
