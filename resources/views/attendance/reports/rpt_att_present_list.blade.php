  ...
  <div class="title">
    <div  style="background-color:#e0e0e0;" class="widget-header widget-header-small">
      <h6 class="widget-title smaller">
        <font size="3" color="blue"><b>Present Report</b></font>
      </h6>
    </div>
  </div>
  <div class="container">
    <form id='myform' action="{{route('get.attend.present.data')}}" id="acc_form" method="post">
      {{ csrf_field() }}
      ...
      <div class="row justify-content-center">
        <div class="col-md-12">
          <table class="table table-striped table-data table-report"> 
            <tbody>
              <?php  $sl=1; $i=0; $i=1; $department = ''; $section = '';?>
              @foreach($rows as $row)  
              <?php if($section != '' && $section != $row->section) {  ?>
                    <tr>
                      <td colspan="9">Section Wise Count : {{ $i-1 }}</td> 
                    </tr>
                  <?php } ?>
