  ...
  <div class="title">
    <div  style="background-color:#e0e0e0;" class="widget-header widget-header-small">
      <h6 class="widget-title smaller">
        <font size="3" color="blue"><b>Leave Report</b></font>
      </h6>
    </div>
  </div>
  <div class="container">
    <form id='myform' action="{{route('get.emp.leave.data')}}" id="acc_form" method="post">
      {{ csrf_field() }}
          ...
          <table class="table table-striped table-data table-report"> 
            <tbody>
              ...
              <?php if($section != '' && $section != $row->section) {  ?>
                    <tr>
                      <td colspan="8">Section Wise Count : {{ $i-1 }}</td> 
                    </tr>
                  <?php } ?>
                ...
                <?php if($section == '' || $section != $row->section) {  
                  ...
                  <tr> 
                    ...
                    <th class="text-right" colspan="5">Section : {{ $row->section }}</th>
                  </tr>  
                <tr>
              ...
              <tr> 
                ...
                <td align="center">{{ date('d-m-Y',strtotime($row->leave_from_dt)) }}</td> 
                <td align="center">{{ date('d-m-Y',strtotime($row->leave_to_dt)) }}</td>  
                <td align="center">{{ $row->leave_days }}</td>
              </tr> 
