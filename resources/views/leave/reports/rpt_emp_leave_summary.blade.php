  ...
  <div class="title">
    <div  style="background-color:#e0e0e0;" class="widget-header widget-header-small">
      <h6 class="widget-title smaller">
        <font size="3" color="blue"><b>Leave Summary Report</b></font>
      </h6>
    </div>
  </div>
  <div class="container">
    <form id='myform' action="{{route('get.emp.leave.summary.data')}}" id="acc_form" method="post">
      {{ csrf_field() }}
            ...
            <tbody>
              ...
              @foreach($rows as $row) 
                <?php if($section != '' && $section != $row->section) {  ?>
                    <tr>
                      <td colspan="10">Section Wise Count : {{ $i-1 }}</td> 
                    </tr>
                  <?php } ?>
                ...
                <?php if($section == '' || $section != $row->section) {  
                  ...
                  <tr> 
                    ...
                    <th class="text-right" colspan="6">Section : {{ $row->section }}</th>
                  </tr>  
                <tr>
              ...
              <tr>
                ...
                <td align="center">{{ number_format($row->EL_DAYS) }}</td>
                <td align="center">{{ number_format($row->CL_DAYS) }}</td> 
                <td align="center">{{ number_format($row->ML_DAYS) }}</td>
                <td align="center">{{ number_format($row->SL_DAYS) }}</td> 
