...
<div class="title">
  <div class="widget-header widget-header-small">
    <font size="3" color="blue"><b>Employee Leave Entry Information</b></font>
...
<div class="widget-body">
  ...
  {{ csrf_field() }}
   <div class="row">
       <div class="col-md-3">
         <a href="{{route('leave.create')}}" class="btn btn-success btn-sm">
    ...
    <div class="col-md-12">
      ...
      <table class="table table-striped table-view">
        <thead class="thead-blue">
          ...
          <th class="text-center" scope="col">Leave</th> 
        ...
        <tbody>
          @foreach($rows as $row) 
            ...
            <td width="8%">{{ $row->leave_type }}</td>
            <td width="20%">{{ date('d-m-Y',strtotime($row->leave_from_dt)) }}</td> 
            <td width="20%">{{ date('d-m-Y',strtotime($row->leave_to_dt)) }}</td>
            <td width="8%">{{ $row->leave_days }}</td> 
            <td width="8%">{{ $row->leave_reasons }}</td>  
            <td width="18%">
              <form  method="post" action="{{ url('/leave/destroy/'.$row->id) }}" class="delete_form">
                {{ csrf_field() }}
                {{ method_field('DELETE') }} 
                <div class="btn-group btn-corner">
                  <a><a href="{{ route('leave.edit',$row->id) }}" class="btn btn-xs btn-primary edit">Edit</a> 
