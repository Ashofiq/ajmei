...
<section class="content">
  ...
  <div  style="background-color:#e0e0e0" class="widget-header widget-header-small">
    ...
    <font size="2" color="blue"><b>Edit: Finish Goods Receive Form</b></font>
    </h6>
...
@endif
  <form id="inv_Form" action="{{route('fin.goods.rec.update')}}" method="post">
    {{ csrf_field() }}
    <input type="hidden" id="id" name="id" value="{{ $rows_m->id }}" class="form-control  input-sm" autocomplete="off" required/>
    <input type="hidden" name="pur_order_no" id="pur_order_no" value="{{ $rows_m->r_issue_order_no}}" class="form-control  input-sm" autocomplete="off" />
    <input type="hidden" name="res_issue_id" id="res_issue_id" value="{{$rows_m->f_rec_issue_prod_id}}" 
    <div class="widget-body">
         ...
         <div class="row"> 
          <div class="col-md-3">
            <div class="input-group ss-item-required">
                ...
                <input type="text" size = "15" name="rec_date" onclick="displayDatePicker('rec_date');"  value={{ date('d-m-Y',strtotime($rows_m->f_rec_order_date)) }} />
                 ...
                 <option value="" >--Select--</option>
                     @if ($companies->count())
                         @foreach($companies as $company)
                             <option {{ $rows_m->f_rec_comp_id == $company->comp_id ? 'selected' : '' }} value="{{$company->comp_id}}" >{{ $company->comp_id }}-{{ $company->name }}</option>
                         @endforeach
                     @endif
                 </select>
                </div>
           </div> 
            <input type="hidden" name="currencyValue" id="currencyValue" value="{{$rows_m->f_rec_m_curr_rate}}" class="form-control changesCurrency" required/>
            <input type="hidden" name="currencyName" id="currencyName" value="{{$rows_m->f_rec_m_curr}}" class="form-control" required readonly/>
      ...
      <div class="row">
        <div class="col-md-3">
          <div class="input-group ss-item-required">
            <div class="input-group-prepend">
              <div class="input-group-text" style="min-width:80px">Rec No:</div>
            </div>
            <input type="text" name="purchase_no" id="purchase_no" value="{{$rows_m->f_rec_order_no}}" class="form-control" required readonly/>
          </div>
        </div> 
      </div>
        ...
        <div class="row">
         <div class="col-md-12">
             <div class="input-group ss-item-required">
                 ...
                 <textarea class="summernote" rows="100" cols="100" name="comments" id="comments">{!! $rows_m->f_rec_comments !!}</textarea>
                 ...
                 <div class="row">
                   <div class="col-md-12 input-group">
                     <table id="salesTable" class="table table-striped table-data table-view">
                       <thead class="salesTable">
                         ...
                         <th width="7%" class="text-center">Weight</th> 
                   ...
                   <tbody class="salesTable" style="background-color: #ffffff;">
                    ...
                    @foreach($rows_d as $d)
                       ...
                       <td width="1%" style="display: none"><input type="text" data-type="IssueProdDetId" name="IssueProdDetId[]" id="IssueProdDetId_{{$i}}" value="{{$d->f_rec_issu_prd_det_id}}" class="form-control" autocomplete="off" readonly></td>
                       ...
                       <td width="20%"><input type="text" data-type="RawItemName" name="RawItemName[]" id="RawItemName_{{$i}}" value="{{$d->raw_item_name}}" class="form-control" autocomplete="off" readonly></td>
                       ...
                       <td width="7%"><input type="text" data-type="BalRawItemQty" name="BalRawItemQty[]" id="BalRawItemQty_{{$i}}" value="{{$d->r_issue_item_qty - $d->r_issue_item_qty_rec + $d->f_rec_item_qty}}" class="form-control" autocomplete="off" readonly></td>
                       ...
                       <td width="2%" style="display: none"><input type="text" data-type="ItemCodeId" name="ItemCodeId[]" id="ItemCodeId_{{$i}}" value="{{$d->f_rec_item_id}}" class="form-control item_id_class" autocomplete="off"></td>
                       <td width="5%" style="display: none"><input type="text" data-type="ItemCode" name="ItemCode[]" id="ItemCode_{{$i}}" value="{{$d->fin_item_code}}" class="form-control autocomplete_txt" autocomplete="off"></td>
                       <td width="7%"  style="display: none" ><input type="text" data-type="ItemBarCode" name="ItemBarCode[]" id="ItemBarCode_{{$i}}" value="{{$d->fin_item_bar_code}}" onKeyUp="loadItemsDetByBarcode(this.id,this.value)" class="form-control autocomplete_barcode_txt" autocomplete="off"></td>
                       <td width="23%">
                       <div><select data-type="itemid" name="itemid[]"  id ="itemid_{{$i}}" class="form-control chosen-select" onchange="loadItemsDet(this.id,this.value)">
                           ...
                           @foreach($item_list as $cmb)
                               <option  {{ $cmb->id == $d->f_rec_item_id ? 'selected' : '' }} value="{{ $cmb->id }}">{{ $cmb->item_name }}({{ $cmb->itm_cat_name }})</option>
                           @endforeach
                         </select></div>
                      </td>
                       <td width="20%"><input type="text" data-type="ItemDesc" name="ItemDesc[]" id="ItemDesc_{{$i}}" value="{{$d->f_rec_item_desc}}"  class="form-control" autocomplete="off" ></td>
                       ...
                       <!-- <td width="8%" align="center"><div><select data-type="Unit" name="Unit[]"  id ="Unit_{{$i}}" class="form-control chosen-select">
                           ...
                           <option  {{ $cmb->vUnitName == $d->f_rec_item_unit ? 'selected' : '' }} value="{{ $cmb->vUnitName }}">{{ $cmb->vUnitName }}</option> 
                           @endforeach
                         </select></div></td> -->

                         <td width="7%" align="center"><input type="text" data-type="PCS" value="{{$d->f_rec_item_pcs}}" name="PCS[]" id="PCS_{{$i}}" class="form-control input-sm changesNo" style="font-weight:bold; text-align: center;" autocomplete="off" ></td>

                       <td width="10%" align="center"><input type="text" data-type="Qty" name="Qty[]" id="Qty_{{$i}}" value="{{$d->f_rec_item_qty}}" onkeydown="enter(this.id,this.value)" class="form-control input-sm changesNo iQty" style="font-weight:bold; text-align: center;" autocomplete="off" ></td>
                       
                       <td width="7%" align="center"><input type="text" data-type="QWeight" name="QWeight[]" id="QWeight_{{$i}}" value="{{$d->f_rec_item_weight}}" class="form-control input-sm changesNo" style="font-weight:bold; text-align: center;" autocomplete="off" ></td>

                      
                      <!-- <td width="7%" align="center"><input type="text" data-type="QWeight" name="QWeight[]" id="QWeight_{{$i}}" value="{{$d->f_rec_item_weight}}" class="form-control input-sm changesNo" style="font-weight:bold; text-align: center;" autocomplete="off" ></td>

                      <td width="7%" align="center"><input type="text" data-type="PCS" name="PCS[]" id="PCS_{{$i}}" value="{{$d->f_rec_item_pcs}}" class="form-control input-sm changesNo" style="font-weight:bold; text-align: center;" autocomplete="off" ></td>

                       <td width="10%" align="center"><input type="text" data-type="Qty" name="Qty[]" id="Qty_{{$i}}" value="{{$d->f_rec_item_qty}}" onkeydown="enter(this.id,this.value)" class="form-control input-sm changesNo iQty" style="font-weight:bold; text-align: center;" autocomplete="off" ></td> -->
                       <td width="5%"><input type="text" data-type="Unit" name="Unit[]" id="Unit_{{$i}}" value="{{$d->f_rec_item_unit}}" class="form-control" autocomplete="off" ></td>
                       
                       <td width="8%" align="right"><input type="text" data-type="Rate" name="Rate[]" id="Rate_{{$i}}" value="{{$d->f_rec_item_price}}" onkeydown="enter(this.id,this.value)" class="form-control input-sm changesNo iRate" style="font-weight:bold; text-align: center;" autocomplete="off"></td>
                       <td width="12%" align="right"><input type="text" data-type="Amount" name="Amount[]" id="Amount_{{$i}}" value="{{$d->f_rec_item_qty * $d->f_rec_item_price}}" class="form-control input-sm iAmount" style="font-weight:bold; text-align: right;" autocomplete="off" readonly></td> 
                       <td width="10%" align="right"><input type="text" data-type="Remarks" name="Remarks[]" id="Remarks_{{$i}}" value="{{$d->f_rec_item_remarks}}" class="form-control input-sm" style="font-weight:bold; text-align: left;" autocomplete="off" ></td>
                 ...
                 <table class="table table-striped table-data table-view">
                   <tbody style="background-color: #ffffff;">
                     <tr>
                       ...
                       <td width="2%" align="center"><input type="text" data-type="total_qty" name="total_qty" id="total_qty" value="{{$rows_m->f_rec_total_qty}}" style="font-weight:bold; text-align: right;" readonly></td>
                       <td width="2%" align="right"><input type="text" data-type="total_amount" name="total_amount" id="total_amount" value="{{$rows_m->f_rec_total_amount}}" style="font-weight:bold; text-align: right;" readonly></td> 
                      <td width="3%"></td>
                     </tr>
                    </tbody>
                 </table>
      ...
      <div class="row justify-content-left">
        <div class="col-sm-12 text-left">
            ...
            <a href="{{route('raw.itm.issue.prod.index')}}" class="btn btn-sm btn-info"><i class="fa fa-list"></i> List</a>
        </div>
      </div>
    </div>
    </div>
...
function loadItemsDet(el,itemid){
    ...
    $.get('/get-fin-goods-rec-code/getdetails/'+itemid+'/getfirst', function(data){
    item = data.data
    }).then(function(){ 
