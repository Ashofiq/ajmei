  ...
  <div class="title">
    <div  style="background-color:#e0e0e0;" class="widget-header widget-header-small">
      ...
      <h6 class="widget-title smaller">
        <font size="3" color="blue"><b>Item Wise Sales Report</b></font>
      </h6>
    </div>
  </div>
<div class="container">
<form action="{{route('rpt.inv.itm.list')}}" method="POST">
  {{ csrf_field() }}
               ...
               <select id="item_id" name="item_id" class="chosen-select" >
                 <option value="" disabled selected>- Select Item -</option>
                   @foreach($item_list as $item)
                     <option {{ old('item_id') == $item->id ? 'selected' : '' }} value="{{ $item->id }}">{{ $item->item_name }}({{$item->itm_cat_origin}}{{$item->itm_cat_name}})</option>
                   @endforeach
               </select>
               @error('item_id')
        ...
        <tbody>
         ...
         ?>
          ...
          <tr>
            ...
            <td align="right">{{ number_format($n_amt,0) }}</td>
          </tr>
          @endforeach
          <tr>
            <td colspan="7">&nbsp;</td> 
