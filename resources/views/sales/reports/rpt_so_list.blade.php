  ...
  <div class="title">
    <div  style="background-color:#e0e0e0;" class="widget-header widget-header-small">
      ...
      <h6 class="widget-title smaller">
        <font size="3" color="blue"><b>Sales Order Report</b></font>
      </h6>
    </div>
  </div>
<div class="container">
<form action="{{route('rpt.so.list')}}" method="POST">
  {{ csrf_field() }}
  <div class="row justify-content-center">
    ...
    <div class="col-md-2">
      <input type="text" name="order_no" id="order_no" value="{{$order_no}}" class="form-control" placeholder="Enter Order No"/>
    </div>
    <div class="col-md-2">
      ...
      <table class="table table-striped table-data table-view">
        ...
        <tbody>
         ...
         ?>
          @foreach($rows as $row)
          <?php 
            $so_sub_total += $row->so_sub_total;
            $so_total_disc += $row->so_total_disc; 
            $so_vat_value += $row->so_vat_value;
            $so_net_amt += $row->so_net_amt;
          ...
          <tr>
            ...
            <td align="right">{{ $row->so_sub_total }}</td>
            <td align="right">{{ $row->so_total_disc }}</td>
            <td align="right">{{ $row->so_vat_value }}</td>
            <td align="right">{{ $row->so_net_amt }}</td>

          </tr>
          @endforeach
          <tr>
            <td colspan="6">&nbsp;</td> 
            <td align="right"><b>{{ number_format($so_sub_total,2) }}</b></td>
            <td align="right"><b>{{ number_format($so_total_disc,2) }}</b></td>
            <td align="right"><b>{{ number_format($so_vat_value,2) }}</b></td>
            <td align="right"><b>{{ number_format($so_net_amt,2) }}</b></td>
          </tr>
          </tbody>
        </table>
        </div>
