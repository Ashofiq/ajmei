                ...
                <header>
                    <table width="100%">
                        ...
                        <tr>
                            ...
                            <td align="center" valign="top" style="width:25%; "><p style="font-size: 20px; text-align: center; font-weight: bold">Sales Bill</p></td>
                            <td valign="top" style="width:25%; "></td>
                        </tr>
                ...
                <div>
                    ...
                    <table width="100%" style="font-family: serif; margin-top:-10px;" cellpadding="10">
                        <tr>
                            ...
                            <td width="10%"> </td>
                            <td width="30%" style="border: 0.1mm solid #888888;">
                                <span>Bill No :</span> {{ $rows_m->inv_no }}<br/> 
                                Bill Date : {{ date('d/m/Y', strtotime($rows_m->inv_date)) }}<br/>
                                Order No : {{ $rows_m->so_order_no }}<br/>
                                Order Date : {{ date('d/m/Y', strtotime($rows_m->so_order_date)) }}<br>
                                FPO No : 000{{ $salseOrder->so_fpo_no }}<br/>
                                FPO Date : {{ date('d/m/Y', strtotime($rows_m->so_order_date)) }}<br>
                    ...
                    <table class="items" width="100%" style="font-size: 9pt; border-collapse: collapse; " cellpadding="8">
                        ...
                        <tr>
                            ...
                            <td width="6%"> Total Weight</td>
                           
                            <!-- <td width="10%">Total Weight (KG)</td> -->
                            <td width="7%">Sales Unit</td>
                            <td width="10%">Unit Price</td>
                            <?php $colspan = 7; $diskCol = false; ?>
                            @foreach($rows_d as $sale_details)
                                
                                @if($sale_details->inv_itm_disc_per > 0.00)
                                <?php $colspan = 9; $diskCol = true; ?>
                                <td width="10%">Disc.</td>
                        ...
                        @foreach($rows_d as $sale_details)
                         <?php 
                           if($sale_details->inv_unit == 'KG'){
                            $sub_total = $sale_details->inv_item_price*$sale_details->inv_qty;
                          }else{
                            $sub_total = $sale_details->inv_item_price*$sale_details->inv_item_pcs;  
                          }
                          ...
                          ?>
                            <tr>
                                ...
                                <td align="left" width="12%" style="border-bottom: 1px solid #cccccc;">
                                <b>{{ $sale_details->itm_cat_name }}</b> <br/>
                                {{ $sale_details->item_name }}<br/>
                                {{ $sale_details->inv_item_spec }}
                                </td>
                                <td align="center" width="10%"
                                    style="border-bottom: 1px solid #cccccc;">
                                    {{ $sale_details->inv_item_size }} {{ ($sale_details->inv_item_size != '') ? 'Inc.' : '' }}
                                </td> 

                                <td align="center" width="5%"
                                    style="border-bottom: 1px solid #cccccc;">
                                    {{ $sale_details->inv_item_weight }} gm</td>
                                
                                <td align="center" width="5%"
                                    style="border-bottom: 1px solid #cccccc;">
                                    {{ intval($sale_details->inv_item_pcs) }}

                                   
                                </td>
                                @if($sale_details->inv_unit == 'KG')
                                <td align="center" width="5%" style="border-bottom: 1px solid #cccccc;">
                                    <?php 
                                        echo number_format($sale_details->inv_qty, 3).' KG';
                                        // if($sale_details->inv_unit == 'KG'){
                                        //     echo number_format($sale_details->inv_qty, 3);
                                        // }else{
                                        //     echo number_format($sale_details->inv_item_weight / 1000, 3);
                                        // }
                                    ?> 
                                    </td>
                                @else
                                    <td  width="5%" style="border-bottom: 1px solid #cccccc;">{{ $sale_details->inv_item_weight }} gm </td>
                                @endif
                                
                                
                                <!-- <td align="center" width="5%"
                                    style="border-bottom: 1px solid #cccccc;">{{ $sale_details->inv_qty }}</td> -->
                                
                                <td align="center" width="5%"
                                    style="border-bottom: 1px solid #cccccc;">
                                    {{ $sale_details->inv_unit }}</td>
                                
                                <td align="center" width="5%"
                                    style="border-bottom: 1px solid #cccccc;">{{ $sale_details->inv_item_price }}</td>
                                ...
                                <td align="right" style="text-align:right; border-bottom: 1px solid #cccccc;"
                                    width="15%">{{ number_format($s_total, 3) }} Tk
                                </td>
                            </tr>
                            <?php
                            $i++; ?>
                          @endforeach
                        <?php
                         $Total_Itm_Disc = $rows_m->inv_total_disc; 
                        //$Total_Disc = $rows_m->inv_total_disc + $rows_m->inv_disc_value;  // total item disc + overll disc 
                        $Total_Disc = $rows_m->inv_disc_value;  // total item disc + overll disc 
                        ...
                        <tr>
                            <td class="totals" colspan="{{ $colspan }}" rowspan="9" align = "left"></td>
                        ...
                        <tr>  
                            ...
                            <td style="text-align:right" class="totals cost">
                                <b>{{ number_format($Total_Disc , 3) }} Tk</b></td>
                        </tr>
                        @endif
                        @if($rows_m->inv_carring_cost != 0)   
                        <tr> 
                            <td class="totals"><b>Carring Cost:</b></td>
                            <td style="text-align:right" class="totals cost">
                                <b>{{ number_format($rows_m->inv_carring_cost , 0) }} Tk</b></td>
                        </tr>
                        @endif
                        @if($rows_m->inv_labour_cost != 0)
                        <tr> 
                            <td class="totals"><b>Labour Cost:</b></td>
                            <td style="text-align:right" class="totals cost">
                                <b>{{ number_format($rows_m->inv_labour_cost , 0) }} Tk</b></td>
                        </tr>
                        @endif
                        @if($rows_m->inv_load_unload_cost != 0)
                        <tr> 
                            <td class="totals"><b>Load/Unload Cost:</b></td>
                            <td style="text-align:right" class="totals cost">
                                <b>{{ number_format($rows_m->inv_load_unload_cost , 0) }} Tk</b></td>
                        </tr>
                        @endif
                        @if($rows_m->inv_service_charge != 0)
                        <tr> 
                            <td class="totals"><b>Service Charge:</b></td>
                            <td style="text-align:right" class="totals cost">
                                <b>{{ number_format($rows_m->inv_service_charge , 0) }} Tk</b></td>
                        </tr>
                        @endif    
                        @if($rows_m->inv_other_cost != 0)
                        <tr> 
                            <td class="totals"><b>Other Cost:</b></td>
                            <td style="text-align:right" class="totals cost">
                                <b>{{ number_format($rows_m->inv_other_cost , 0) }} Tk</b></td>
                        </tr>
                        @endif     
                        <!--<tr> -->
                        <!--    <td class="totals"><b>Payable:</b></td>-->
                        <!--    <td style="text-align:right" class="totals cost">-->
                        <!--        <b>{{ number_format($inv_net_amt , 0) }} Tk</b></td>-->
                        <!--</tr>-->
                        ...
                        <tr> 
                            ...
                            <td style="text-align:right" class="totals cost">
                                 <!--<b><?php //echo number_format(($inv_net_amt),2); ?> Tk</b>-->
                                 <b><?php echo number_format(($inv_net_amt - $rows_m->so_special_offer),2); ?> Tk</b>
                                <!--<b>{{ floatval(number_format($inv_net_amt , 0)) - intval($rows_m->so_special_offer) }} Tk</b>-->
