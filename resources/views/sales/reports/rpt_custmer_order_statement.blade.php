  ...
  <div class="title">
    <div  style="background-color:#e0e0e0;" class="widget-header widget-header-small">
      <h6 class="widget-title smaller">
        <font size="3" color="blue"><b>Customer Ledger Sheet</b></font>
      </h6>
    </div>
  </div>
  <div class="container">
    <form id='myform' action="{{route('rpt.cust.order.statement')}}"  method="post">
      {{ csrf_field() }}
        ...
        <div class="col-md-12">
          <table class="table table-striped table-data table-view">
            ...
            <tbody>
              ...
              $order_amount += ($row->trans_type == 'SO') ? $row->c_amount : 0;
              $deposit += ($row->trans_type != 'SO') ? $row->c_amount : $row->c_amount;
              $invAmount += ($row->trans_type != 'SO') ? $row->d_amount : $row->d_amount;
              ?>
              <tr>
                <td width="5%">{{ date('d-m-Y', strtotime($row->voucher_date)) }} </td>
                <td width="25%">{!! $row->t_narration !!}</td>
                <td width="8%" align="center">{{ $row->trans_type }}-{{ $row->voucher_no }}</td>
                <td width="5%" align="center">
                  @if($row->trans_type == 'SO')
                    {{ number_format($row->c_amount,2)=='0.00'?'':number_format($row->c_amount,2) }}
                  @endif
                ...
                <!-- <td width="5%" align="center">{{ ($row->acc_invoice_no == 0) ? '' :  $row->acc_invoice_no }}</td> -->
                <td width="8%" align="right">
                  @if($row->trans_type != 'SO')
                    {{ number_format($row->d_amount,2)=='0.00'?'':number_format($row->d_amount,2) }}
                  @endif
                </td>
              ...
              <tr>
                ...
                <!-- <td width="8%" align="right"><b>{{ number_format($order_amount,2) }}</b></td>
                <td width="8%" align="right"><b>{{ number_format($deposit,2) }}</b></td>
                <td width="8%" align="right"><b>{{ number_format($invAmount,2) }}</b></td> -->
                <td width="8%" align="right"><b>{{ number_format($balance,2) }}</b></td>
              </tr>
              <tr>
