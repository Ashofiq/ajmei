  ...
  <div class="title">
    <div  style="background-color:#e0e0e0;" class="widget-header widget-header-small">
      ...
      <h6 class="widget-title smaller">
        <font size="3" color="blue"><b>Daily Delivery Report</b></font>
      </h6>
    </div>
  </div>
<div class="container">

<form action="{{route('sales.delivery.prod.report.daily')}}" method="post">
  {{ csrf_field() }}
   <div class="row"> 
        <div class="col-md-2">
    ...
    <div class="col-md-12">
      <table class="table table-striped table-data table-view">
        <thead class="thead-blue">
          ...
          <th class="text-center" scope="col">Delivery Psc</th>
          <th class="text-center" scope="col">Delivery KG</th>
        ...
        <tbody>
          ...
          <tr>
            ...
            <td>{{ $row->orderPcs }}</td>
            <td>{{ $row->orderKg }}</td>
            <td>{{ $row->del_item_pcs }}</td>
            <td>{{ $row->del_item_kg }}</td>
            <td>{{  $row->orderPcs - $row->del_item_pcs }}</td>
            <td>{{ $row->orderKg - $row->del_item_kg }}</td>
          </tr>
          @endforeach
