  ...
  <div class="title">
    <div  style="background-color:#e0e0e0;" class="widget-header widget-header-small">
      ...
      <h6 class="widget-title smaller">
        <font size="3" color="blue"><b>Return List</b></font>
      </h6>
    </div>
  </div>
<div class="container">
<form action="{{route('sales.return.index')}}" method="POST">
  {{ csrf_field() }}
   ...
   <div class="col-md-2">
    ...
    <div class="col-md-1">
        <div class="form-group">
          <input type="text" size = "8" name="fromdate" onclick="displayDatePicker('fromdate');"  value="{{ $fromdate!=''?date('d-m-Y',strtotime($fromdate)):'' }}" />
    ...
    <div class="col-md-1">
        <div class="form-group">
          <input type="text" size = "8" name="todate" onclick="displayDatePicker('todate');"   value="{{ $fromdate!=''?date('d-m-Y',strtotime($todate)):'' }}" />
    ...
    <div class="col-md-12">
      <table class="table table-striped table-data table-view">
        <thead class="thead-blue">
          ...
          <th class="text-center" scope="col">Return No</th>
          <th class="text-center" scope="col">Return Date</th>
          <th class="text-center" scope="col">Acc No</th> 
        ...
        <tbody>
          ...
          <?php 
               $totalAmount = $row->ret_sub_total + $row->ret_itm_disc_value;
               $netAmount = $row->ret_net_amt;
          ?>
          <tr>
            ...
            <td>{{ $row->inv_no }}</td>
            <td>{{ $row->cust_name }}</td>
            <td>{{ $row->ret_sub_total + $row->ret_itm_disc_value }}</td>
            <td>{{ $row->ret_itm_disc_value + $row->ret_inv_disc_value }}</td>
            <td>{{ $row->ret_vat_value }}</td>
            <td>{{ $row->ret_net_amt }}</td>
            <td>
              <form  method="post" action="{{ url('/return-order/destroy/'.$row->id.'/'.$row->ret_no) }}" class="delete_form">
              {{ csrf_field() }}
              {{ method_field('DELETE') }}
              <div class="btn-group btn-corner">
                <span href="{{ url('/') }}/sales/return-m-view/{{$row->id}}" data-toggle="modal" data-id="{{$row->id}}" class="btn btn-sm btn-info viewModal"
                title="View Details" data-placement="top" >View
                </span>
                <a href="{{ route('sales.return.edit',$row->id) }}" class="btn btn-xs">Edit</a>
                <span href="{{ url('/') }}/return/acc-m-view/{{$row->ret_no}}" data-toggle="modal" data-id="{{$row->inv_no}}" class="btn btn-sm btn-success viewModal" title="View">
                    Acc.Doc
                </span>
