...
<div class="title">
  <div  style="background-color:#e0e0e0" class="widget-header widget-header-small">
    ...
    <font size="2" color="blue"><b>Edit::Sales Return Entry Form</b></font>
    </h6>
...
@endif
  <form id="so_Form" action="{{route('sales.return.update')}}" method="post">
    {{ csrf_field() }}
    <input type="hidden" id="id" name="id" value="{{ $rows->id }}" class="form-control  input-sm" autocomplete="off" required/>
    <input type="hidden" id="inv_id" name="inv_id" value="{{ $rows->ret_sal_inv_id }}" class="form-control  input-sm" autocomplete="off" required/>
    <input type="hidden" id="so_id" name="so_id" value="{{ $rows->ret_sal_ord_id }}" class="form-control  input-sm" autocomplete="off" required/>
    <input type="hidden" id="del_id" name="del_id" value="{{ $rows->ret_sal_del_id }}" class="form-control  input-sm" autocomplete="off" required/>
    <input type="hidden" id="return_orderno" name="return_orderno" value="{{ $rows->ret_no }}" class="form-control  input-sm" autocomplete="off" required/>
    <input type="hidden" name="itm_warehouse" id="itm_warehouse" value="{{ $rows->ret_m_warehouse_id}}" class="form-control  input-sm" autocomplete="off" required/>
        ...
        <div class="col-md-5">
            <div class="input-group">
              <select name="customer_id" id="customer_id" class="chosen-select"  onchange="getCustomerDetails(this.value)">
                <option  value="{{ $rows->ret_cust_id }}">{{ $rows->cust_name }}</option>
              </select>
              @error('customer_id')
      ...
      <div class="row">
         <div class="col-md-8">
           <div class="input-group">
             ...
             <textarea type="text" name="comments" id="comments" class="form-control"/>{{$rows->ret_comments}}</textarea>
           </div>
         </div>
       </div>
       <div class="row">
               ...
               <div role="tabpanel" class="tab-pane active" id = "itemdetails">
                 <div class="row">
                   ...
                   <tbody class="salesTable" style="background-color: #ffffff;">
                    ...
                    @foreach($rows_d as $d)
                    <?php
                      $item_value= $d->ret_qty*$d->ret_item_price-$d->ret_disc_per;
                    ?>
                    <tr>
                      <td width="1.5%" class="text-center">{{$i}}</td>
                      <td width="2%" style="display: none"><input type="text" data-type="ItemCodeId" name="ItemCodeId[]" id="ItemCodeId_{{$i}}" value="{{$d->ret_item_id}}" class="form-control item_id_class" autocomplete="off"></td>
                      ...
                      <div class="input-group ss-item-required">
                        <select data-type="itemid" name="itemid[]"  id ="itemid_{{$i}}" class="form-control chosen-select" onchange="loadItemsDet(this.id,this.value)">
                          ...
                          <option value="{{ $d->ret_item_id }}">{{ $d->item_name }}({{ $d->itm_cat_name }})</option>
                      ...
                      <td width="5%" style="display: none" align="center">
                        <div class="input-group ss-item-required"><select data-type="Storage" name="Storage[]"  id ="Storage_{{$i}}" class="form-control chosen-select" onchange="getDropdownLotList(this.id,this.value)">
                          <option  value="{{ $d->ret_storage_id }}">{{ $d->ret_storage_id }}</option>
                      ...
                      <td width="10%">
                        <div class="input-group ss-item-required"><select data-type="lotno" name="lotno[]"  id ="lotno_{{$i}}" class="form-control chosen-select" onchange="loadLotDet(this.id,this.value)">
                          <option  value="{{ $d->ret_lot_no }}">{{ $d->ret_lot_no }}</option>
                        </select></div>
                      </td>
                      <td width="6%" align="center"><input type="text" data-type="Price" name="Price[]" id="Price_{{$i}}" value="{{$d->ret_item_price}}" class="form-control input-sm" style="font-weight:bold; text-align: center;" autocomplete="off" readonly/></td>
                      ...
                      <td width="5%" align="center"><input type="text" data-type="SQty" name="SQty[]" id="SQty_{{$i}}" value="{{$d->inv_qty - $d->total_ret_qty +$d->ret_qty}}" onkeydown="enter(this.id,this.value)" class="form-control input-sm" style="font-weight:bold; text-align: center;" autocomplete="off" readonly /></td>
                      <td width="5%" align="center"><input type="text" data-type="Qty" name="Qty[]" id="Qty_{{$i}}" value="{{$d->ret_qty}}" onkeydown="enter(this.id,this.value)" class="form-control input-sm changesNo iQty" style="font-weight:bold; text-align: center;" autocomplete="off" /></td>
                      <td width="4%" align="center"><input type="text" data-type="Unit" name="Unit[]" id="Unit_{{$i}}"  value="{{$d->ret_item_unit}}" class="form-control input-sm"  autocomplete="off" readonly/></td>
                      <td width="4%" align="right"><input type="text" data-type="Discp" name="Discp[]" id="Discp_{{$i}}" value="{{$d->ret_disc_per}}" onkeydown="enter(this.value,this.value)" class="form-control input-sm changesNo iDiscp" style="font-weight:bold; text-align: center;" autocomplete="off" readonly/></td>
                      <td width="8%" align="right"><input type="text" data-type="Discount" name="Discount[]" id="Discount_{{$i}}" value="{{number_format($d->ret_disc_value,2)}}" class="form-control input-sm iDiscount"  style="font-weight:bold; text-align: right;" autocomplete="off" readonly/></td>
                      <td width="8%" align="right"><input type="text" data-type="Total" name="Total[]" id="Total_{{$i}}" value="{{number_format($item_value,2)}}" class="form-control input-sm iTotal" style="font-weight:bold; text-align: right;" autocomplete="off" readonly/></td>
                    ...
                    <?php $i = $i + 1;
                      $inv_disc_value = $d->ret_disc_value;
                      $inv_disc_per = $d->ret_disc_per;
                    ?>
                 ...
                 <table class="table table-striped table-data table-view">
                   ...
                   <tbody style="background-color: #ffffff;">
                    ...
                    <tr>
                      ...
                      <td width="12%"><input type="text" id="n_sub_total"  name="n_sub_total" value="{{ $rows->ret_sub_total }}" style="background-color: rgba(255,246,125); font-weight:bold; text-align: right; font-size: 16px;" class="form-control input-sm" placeholder="Sub Total" autocomplete="off" readonly/></td>
                      <td width="5%"><input type="text" id="n_disc_per" name="n_disc_per" value="{{ $rows->ret_itm_disc_per }}" class="form-control input-sm changesNo" placeholder="Disc(%)" autocomplete="off" readonly/></td>
                      <td width="8%"><input type="text" id="n_discount" name="n_discount" value="{{ $rows->ret_itm_disc_value }}" class="form-control input-sm changesNo" style="font-weight:bold; text-align: right; font-size: 16px;" placeholder="Discount" autocomplete="off" readonly/></td>
                      <td width="15%" ><input type="text" id="n_total_disc" name="n_total_disc" value="{{ $rows->ret_inv_disc_per }}" style="background-color: rgba(255,246,125); font-weight:bold; text-align: right; font-size: 16px;" class="form-control input-sm" placeholder="Total Discount" autocomplete="off" readonly/></td>
                      <td width="15%" ><input type="text" id="n_total_gross" name="n_total_gross" value="{{ $rows->ret_inv_disc_value }}" style="background-color: rgba(195, 248, 150); font-weight:bold; text-align: right; font-size: 16px;" class="form-control input-sm" placeholder="Total Gross" autocomplete="off" readonly/></td>
                      <td width="5%"><input type="text" id="n_vat_per" name="n_vat_per" value="{{ $rows->ret_vat_per }}" class="form-control input-sm changesNo" placeholder="" autocomplete="off" readonly/></td>
                      <td width="12%"><input type="text" id="n_total_vat" name="n_total_vat" value="{{ $rows->ret_vat_value }}" class="form-control input-sm" placeholder="Total VAT" style="font-weight:bold; text-align: right; font-size: 16px;" autocomplete="off" readonly/></td>
                      <td width="15%" ><input type="text" id="n_net_amount" name="n_net_amount" value="{{ $rows->ret_net_amount }}" style="background-color: rgba(255,246,125); font-weight:bold; text-align: right; font-size: 16px;" class="form-control input-sm" placeholder="Net Amount" autocomplete="off" readonly/></td>
                   </tr>
                 </tbody>
                 </table>
               </div>
