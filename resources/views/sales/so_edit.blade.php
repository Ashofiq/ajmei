...
@endif
  ...
  <form id="so_Form" action="{{route('sales.order.update')}}" method="post">
    {{ csrf_field() }}
                   ...
                   <div class="col-md-12 input-group">
                     <table id="salesTable" class="table table-striped table-data table-view">
                       ...
                       <th width="1.5%" class="text-center">&nbsp;&nbsp;</th>
                         ...
                         <th style="display: none" class="text-center">Code</th>
                         <th width="12%" class="text-center">Category</th>
                         <th width="12%" class="text-center">Item Category</th>
                   ...
                   <tbody class="salesTable" style="background-color: #ffffff;">
                    ...
                    ?>
                     <tr>
                       ...
                       <td style="display: none"><input type="text" data-type="ItemCodeId" name="ItemCodeId[]" id="ItemCodeId_{{$i}}" value="{{$d->so_item_id}}" class="form-control item_id_class" autocomplete="off"></td>
                       <td style="display: none"><input type="text" data-type="ItemCode" name="ItemCode[]" id="ItemCode_{{$i}}" value="{{$d->item_code}}" class="form-control autocomplete_txt" autocomplete="off"></td>
                      <td width="12%">
                        <div><select name="itmcategory[]" required id ="itmcategory_1" class="col-xs-10 col-sm-8 chosen-select" onchange="getChildCat(this.id, this.value)" required>
                          <option value="">- Select Category -</option>
                          @foreach($itm_cat as $cat)
                              <option {{ ($cat->id == $d->so_item_cat_id) ? 'selected' : '' }} value="{{ $cat->id }}">{{ $cat->itm_cat_name }}</option>
                          @endforeach
                          </select></div>
                      </td>
                      ...
                      <td width="12%">
                          <div>
                            <select onkeydown="enter(this.id,this.value)" required name="catid[]"  id ="catid_1" class="form-control chosen-select" onchange="loadItemList(this.id,this.value)" >
                              <option value="" >- Select Category -</option>
                              @foreach($item_list as $cmb)
                                  <option {{ ($cmb->cat_id == $d->so_item_cat_2nd_id) ? 'selected' : '' }} value="{{ $cmb->id }}">{{ $cmb->item_name }}({{ $cmb->itm_cat_name }})</option>
                              @endforeach
                            </select>
                          </div>
                      </td>

                       <td width="18%"><input type="text" data-type="newItemName" name="newItemName[]" id="newItemName{{$i}}" value="{{$d->item_name}}" class="form-control" autocomplete="off" readonly></td>
...
function getChildCat(id, catId) {
    console.log(id);
    field = id.split("_")[1];
    $.get('{{ url('/') }}/get-child-cat/'+catId, function(data){
