...
<form action="{{route('sales.invoice.locked')}}" method="POST">
  {{ csrf_field() }}
...
<form action="{{route('sales.invoice.locking')}}" method="POST">
  {{ csrf_field() }}
  <div class="row justify-content-center">
    <div class="col-md-12">
      <table class="table table-striped table-data table-view">
        <thead class="thead-blue">
          ...
          <th class="text-center" scope="col">
              <button type="submit" name="submit" value='html'  class="btn btn-sm btn-info"><span class="fa fa-inf">Locked</span></button>
          </th>
        </thead>
        <tbody>
          @foreach($rows as $row)
          <tr>
            ...
            <td>{{ $row->so_order_no }}
            <input type="hidden" data-type="so_order_no" name="so_order_no[]" id="so_order_no" value="{{ $row->so_order_no }}" class="form-control item_id_class" autocomplete="off"></td>
            ...
            <td>{{$row->so_is_locked == 1?'Locked':' '}}
              <!--div class="btn-group btn-corner">
                ...
                <span href="{{ url('/') }}/invoice/acc-m-view/{{$row->inv_no}}" data-toggle="modal" data-id="{{$row->inv_no}}" class="btn btn-sm btn-success viewModal" title="View">
                    Acc.Doc
                </span>
