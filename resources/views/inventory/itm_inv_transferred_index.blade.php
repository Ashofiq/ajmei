  ...
  <div class="title">
    ...
    <div class="widget-header widget-header-small">
        <h6 class="widget-title smaller">
          <font size="3" color="blue"><b>Inventory Transfer List</b></font>
        </h6>
...
<div class="container">
  ...
  <form action="{{route('itm.inv.transfer.index')}}" method="post">
  {{ csrf_field() }}
   <div class="row">
       <div class="col-md-2">
         <a href="{{route('itm.inv.transfer.create')}}" class="btn btn-success btn-sm">
    ...
    <div class="col-md-12">
      <table class="table table-striped table-data table-report">
        ...
        <tbody>
          ...
          <tr>
            <td>{{ $row->id }}</td>
            <td>{{ $row->s_ware_name }}</td>
            <td>{{ $row->r_ware_name }}</td>
            <td>{{ date('d-m-Y',strtotime($row->trans_date)) }}</td>
            <td>{{ $row->trans_comments }}</td>
            <td>{{ $row->trans_total_qty }}</td>
            <td>
                <form  method="post" action="{{ url('/') }}/itm-inv-transfer/destroy/{{$row->id }}" class="delete_form">
                {{ csrf_field() }}
                {{ method_field('DELETE') }}
                <div class="btn-group btn-corner">
                  <span href="{{ url('/') }}/itm-inv-transfer/item-m-view/{{$row->id}}" data-toggle="modal" data-id="{{$row->id}}"
                ...
                @else
                <span href="#" data-toggle="modal"  class="btn btn-sm btn-info" title="Done" data-placement="top" >
                   Received Done
                </span>
                @endif
                </div>
