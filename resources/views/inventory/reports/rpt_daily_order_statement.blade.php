  ...
  <div class="title">
    <div  style="background-color:#e0e0e0;" class="widget-header widget-header-small">
      <h6 class="widget-title smaller">
        <font size="3" color="blue"><b>Daily Cash Book</b></font> 
      </h6>
    </div>
  </div>
  <div class="container">
    <form id='myform' action="{{route('rpt.daily.order.statement')}}"  method="post">
      {{ csrf_field() }}
          ...
          <table class="table table-striped table-data table-view">
            ...
            <tbody>
              ...
              if($row->trans_type == 'SO'){
                $orderAmount += $row->c_amount;
              }

              if($row->trans_type != 'SO'){
                $deposite += $row->c_amount;
              }

              if($row->trans_type != 'SO'){
                $invoiceAmount += $row->d_amount;
              }
              
              ?>
              <tr>
                <td width="5%">{{ date('d-m-Y', strtotime($row->voucher_date)) }} </td>
                <td width="25%" align="center">{{ $row->party }}</td>
                <td width="8%" align="center">{{ $row->trans_type }}</td>
              ...
              <tr>
                ...
                <td width="8%" align="right"><b>{{$orderAmount }}</b></td>
                <td width="8%" align="right"><b>{{$deposite }}</b></td>
                <td width="8%" align="right"><b>{{$invoiceAmount }}</b></td>
