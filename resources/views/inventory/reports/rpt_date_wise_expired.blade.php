  ...
  <div class="title">
    <div  style="background-color:#e0e0e0;" class="widget-header widget-header-small">
      <h6 class="widget-title smaller">
        <font size="3" color="blue"><b>Date Wise Expired Report</b></font>
      </h6>
    </div>
  </div>
  <div class="container">
    <form id='myform' action="{{route('itm.dt.wise.expired.rpt')}}" id="acc_form" method="post">
      {{ csrf_field() }}
          ...
          <table class="table table-striped table-data table-report">
            ...
            <tbody>
              ...
              <tr>
                ...
                <td>{{ $row->item_name }}</td>
                <td>{{ $row->exp_lot_no }}</td>
                <td align="right">{{ number_format($row->exp_item_qty,2) }}</td>
              </tr>
              @endforeach
