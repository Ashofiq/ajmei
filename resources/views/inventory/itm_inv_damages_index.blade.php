  ...
  <div class="title">
    ...
    <div class="widget-header widget-header-small">
        <h6 class="widget-title smaller">
          <font size="3" color="blue"><b>Inventory Damages List</b></font>
        </h6>
...
<div class="container">
  ...
  <form action="{{route('itm.inv.dam.index')}}" method="post">
  {{ csrf_field() }}
   <div class="row">
       <div class="col-md-2">
         <a href="{{route('itm.inv.dam.create')}}" class="btn btn-success btn-sm">
      ...
      <table class="table table-striped table-data table-report">
        ...
        <tbody>
          ...
          <tr>
            <td>{{ $row->id }}</td>
            <td>{{ $row->ware_name }}</td>
            <td>{{ date('d-m-Y',strtotime($row->dam_date)) }}</td>
            <td>{{ $row->dam_comments }}</td>
            <td>{{ $row->dam_total_qty }}</td>
            <td>

                <form  method="post" action="{{ url('/') }}/itm-inv-dam/destroy/{{$row->id }}" class="delete_form">
                {{ csrf_field() }}
                {{ method_field('DELETE') }}
                <div class="btn-group btn-corner">
                  <span href="{{ url('/') }}/itm-inv-dam/item-m-view/{{$row->id}}" data-toggle="modal" data-id="{{$row->id}}"
                      class="btn btn-sm btn-info viewModal" title="View Details" data-placement="top" >
                     View
                  </span>
                  <a href="{{ route('itm.inv.dam.edit',$row->id) }}" class="btn btn-xs">Edit</a>
