...
<section class="content">
  ...
  <div  style="background-color:#e0e0e0" class="widget-header widget-header-small">
    ...
    <font size="2" color="blue"><b>Edit:Inventory Damage Form</b></font>
    </h6>
    <div class="widget-toolbar">
      <a href="{{route('itm.inv.dam.index')}}" class="blue"><i class="fa fa-list"></i> List</a>
    </div>
  </div>
</div>
...
@endif
  <form id="inv_Form" action="{{route('itm.inv.dam.update')}}" method="post">
    {{ csrf_field() }}
    ...
    <div class="widget-body">
         ...
         <div class="row">
          <div class="col-md-3">
            <div class="input-group ss-item-required">
                ...
                <input type="text" size = "15" name="dam_date" onclick="displayDatePicker('dam_date');"  value={{ date('d-m-Y',strtotime($rows_m->dam_date)) }} />
                <a href="javascript:void(0);" onclick="displayDatePicker('dam_date');"><img src="{{ asset('assets/images/calendar.png') }}" alt="calendar" border="0"></a>
            </div>
          </div>
          <div class="col-md-4">
                 ...
                 <option value="" >--Select--</option>
                     @if ($companies->count())
                         @foreach($companies as $company)
                             <option {{ $rows_m->dam_comp_id == $company->comp_id ? 'selected' : '' }} value="{{$company->comp_id}}" >{{ $company->comp_id }}-{{ $company->name }}</option>
                         @endforeach
                     @endif
                 </select>
                </div>
        ...
        <div class="row">
            <div class="col-md-10">
                <div class="input-group ss-item-required">
                    ...
                    <textarea name="comments" rows="2" cols="100" class="form-control config" maxlength="500" required>{{ $rows_m->dam_comments }}</textarea>
                </div>
             </div>
        </div>
        <div class="row">
                 ...
                 <option value="" >--Select Wearhouse--</option>
                     @if ($warehouse_list->count())
                         @foreach($warehouse_list as $list)
                             <option {{ $rows_m->dam_m_warehouse_id == $list->w_ref_id ? 'selected' : '' }} value="{{$list->w_ref_id}}" >{{ $list->ware_name }}</option>
                         @endforeach
                     @endif
                 </select>
                </div>
           ...
           <div class="col-md-3">
             <div class="input-group ss-item-required">
               <input type="hidden" name="result_wh_id" id="result_wh_id" value="{{ $rows_m->dam_m_warehouse_id }}" class="form-control" readonly required/>
               <input type="hidden" name="result_storage_id" id="result_storage_id" value="{{$rows_m->dam_m_warehouse_id}}" class="form-control" readonly required/>
             </div>
           </div>
         </div>
       <div class="row">
               ...
               <div role="tabpanel" class="tab-pane active" id = "itemdetails">
                 <div class="row">
                   ...
                   <tbody class="salesTable" style="background-color: #ffffff;">
                    ...
                    @foreach($rows_d as $d)
                    <?php  
                      $s_total_amt = $d->dam_item_price*$d->dam_item_qty; 
                    ?>
                       <tr>
                       <td width="1.5%" class="text-center">{{$i}}</td>
                       <td width="2%" style="display: none"><input type="text" data-type="ItemCodeId" name="ItemCodeId[]" id="ItemCodeId_{{$i}}" value="{{$d->dam_item_id}}" class="form-control item_id_class" autocomplete="off"></td>
                       ...
                       <div><select data-type="itemid" name="itemid[]"  id ="itemid_{{$i}}" class="form-control chosen-select" onchange="loadItemsDet(this.id,this.value)">
                           ...
                           @foreach($item_list as $cmb)
                               <option  {{ $cmb->id == $d->dam_item_id ? 'selected' : '' }} value="{{ $cmb->id }}">{{ $cmb->item_name }}({{ $cmb->itm_cat_name }})</option>
                           @endforeach
                         </select></div>
                      </td>
                       <td width="18%"><input type="text" data-type="ItemDesc" name="ItemDesc[]" id="ItemDesc_{{$i}}" value="{{$d->dam_item_spec}}"  class="form-control" autocomplete="off" ></td>  
                       <td width="4%" align="center"><input type="text" data-type="Unit" name="Unit[]" id="Unit_{{$i}}" value="{{$d->dam_item_unit}}" class="form-control input-sm"  autocomplete="off" readonly></td>
                       <td width="6%" align="center" ><input type="text" data-type="Stock" name="Stock[]" id="Stock_{{$i}}"  value="{{$d->stock + $d->dam_item_qty}}" class="form-control input-sm" style="font-weight:bold; text-align: center;" autocomplete="off" readonly></td>
                       <td width="6%" style="display: none" align="center"><input type="text" data-type="OldQty" name="OldQty[]" id="OldQty_{{$i}}" value="{{$d->dam_item_qty}}" class="form-control input-sm" style="font-weight:bold; text-align: center;" autocomplete="off" ></td>
                       <td width="6%" align="center"><input type="text" data-type="Qty" name="Qty[]" id="Qty_{{$i}}" value="{{$d->dam_item_qty}}" onkeydown="enter(this.id,this.value)" class="form-control input-sm changesNo iQty" style="font-weight:bold; text-align: center;" autocomplete="off" ></td>
                       <td width="6%" align="center"><input type="text" data-type="Rate" name="Rate[]" id="Rate_{{$i}}" value="{{$d->dam_item_price}}" class="form-control input-sm changesNo iRate" onkeydown="enter(this.id,this.value)" style="font-weight:bold; text-align: center;" class="form-control input-sm" autocomplete="off" ></td>
                       ...
                       <td width="15%" align="right"><input type="text" data-type="Remarks" name="Remarks[]" id="Remarks_{{$i}}" value="{{$d->dam_item_remarks}}" onkeydown="enter(this.id,this.value)" class="form-control input-sm" style="font-weight:bold; text-align: left;" autocomplete="off" ></td>
                 ...
                 <table class="table table-striped table-data table-view">
                   <tbody style="background-color: #ffffff;">
                     <tr>
                       ...
                       <td width="2%" align="center"><input type="text" data-type="total_qty" name="total_qty" id="total_qty" value="{{$rows_m->dam_total_qty}}" style="font-weight:bold; text-align: right;" readonly></td>
                       <td width="2%" align="right"><input type="text" data-type="total_amount" name="total_amount" id="total_amount" value="{{$rows_m->dam_total_amount}}" style="font-weight:bold; text-align: right;" readonly></td> 
                      <td width="3%"></td>
                       <td width="3%"></td>
                     </tr>
                    </tbody>
      ...
      <div class="row justify-content-left">
        <div class="col-sm-12 text-left">
            ...
            <a href="{{route('itm.inv.dam.index')}}" class="btn btn-sm btn-info"><i class="fa fa-list"></i> List</a>
        </div>
      </div>
    </div>
    </div>
