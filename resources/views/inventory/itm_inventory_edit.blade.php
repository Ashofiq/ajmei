...
@endif
  <form id="inv_Form" action="{{route('itm.inventory.update')}}" method="post">
    {{ csrf_field() }}
    <input type="hidden" id="id" name="id" value="{{ $rows_m->id }}" class="form-control  input-sm" autocomplete="off" required/>
    <input type="hidden" name="acc_voucher_no" id="acc_voucher_no" value="{{ $row_acc_vh }}" class="form-control  input-sm" autocomplete="off" />
    <input type="hidden" name="pur_order_no" id="pur_order_no" value="{{ $rows_m->pur_order_no}}" class="form-control  input-sm" autocomplete="off" />
         ...
         <div class="row">
          <div class="col-md-3">
            <div class="input-group ss-item-required">
                ...
                <input type="text" size = "15" name="purchase_date" onclick="displayDatePicker('purchase_date');"  value={{ date('d-m-Y',strtotime($rows_m->pur_order_date)) }} />
                 ...
                 <option value="" >--Select--</option>
                     @if ($companies->count())
                         @foreach($companies as $company)
                             <option {{ $rows_m->pur_comp_id == $company->comp_id ? 'selected' : '' }} value="{{$company->comp_id}}" >{{ $company->comp_id }}-{{ $company->name }}</option>
                         @endforeach
                     @endif
                 </select>
                </div>
      ...
      <div class="row">
        <div class="col-md-3">
          <div class="input-group ss-item-required">
            ...
            <input type="text" name="purchase_no" id="purchase_no" value="{{$rows_m->pur_order_ref}}" class="form-control" required />
            <input type="hidden" name="res_purchase_no" id="res_purchase_no" value="{{$rows_m->pur_order_refid}}" class="form-control" required />
            <input type="hidden" name="currencyValue" id="currencyValue" value="{{$rows_m->pur_m_curr_rate}}" class="form-control changesCurrency" required/>
          </div>
        </div>
        ...
        <div class="col-md-5">
            <div class="input-group ss-item-required">
              <select name="supplier_id" id="supplier_id" class="chosen-select"  onchange="getSupplierDetails(this.value)">
                  ...
                  @foreach($suppliers as $supplier)
                      <option {{ $rows_m->pur_supp_id == $supplier->id ? 'selected' : '' }} value="{{ $supplier->id }}">{{ $supplier->supp_name }}</option>
                  @endforeach
              </select>
              @error('supplier_id')
          ...
          <div class="col-md-3">
            <div class="input-group ss-item-required">
              ...
              <input type="text" name="result_supplier_id" id="result_supplier_id" value="{{$rows_m->pur_supp_id}}" class="form-control" readonly required/>
            </div>
          </div>
        </div>
        <div class="row">
          <div class="col-md-3">
            <div class="input-group ss-item-required">
              ...
              <input type="text" name="pi_no" id="pi_no" value="{{ $rows_m->pur_pi_no }}" class="form-control" />
            </div>
          </div>
            <div class="col-md-8">
                <div class="input-group ss-item-required">
                    ...
                    <textarea name="comments" rows="2" cols="100" class="form-control config" maxlength="500" required>{{ $rows_m->pur_comments }}</textarea>
                </div>
             </div>
        </div>
        <div class="row">
               ...
               <option value="" >--Select Wearhouse--</option>
                   @if ($warehouse_list->count())
                       @foreach($warehouse_list as $list)
                           <option {{ $rows_m->pur_m_warehouse_id == $list->w_ref_id ? 'selected' : '' }} value="{{$list->w_ref_id}}" >{{ $list->ware_name }}</option>
                       @endforeach
                   @endif
               </select>
              </div>
         ...
         <div class="col-md-3">
           <div class="input-group ss-item-required">
             <input type="hidden" name="result_storage_id" id="result_storage_id" value="{{$row_wid->pur_storage_id}}" class="form-control" readonly required/>
           </div>
         </div>
       </div><br/>
       <div class="row">
                 ...
                 <div class="row">
                   <div class="col-md-12 input-group">
                     <table id="salesTable" class="table table-striped table-data table-view">
                       <thead class="salesTable">
                         ...
                         <th width="6%" class="text-center">Order Qty</th>
                   ...
                   <tbody class="salesTable" style="background-color: #ffffff;">
                    ...
                    @foreach($rows_d as $d)
                       ...
                       <td width="2%"style="display: none"><input type="text" data-type="PurorderDetId" name="PurorderDetId[]" id="PurorderDetId_{{$i}}" value="{{$d->pur_order_det_id}}"  class="form-control item_id_class" autocomplete="off"></td>
                       <td width="2%" style="display: none"><input type="text" data-type="ItemCodeId" name="ItemCodeId[]" id="ItemCodeId_{{$i}}" value="{{$d->pur_item_id}}" class="form-control item_id_class" autocomplete="off"></td>
                       ...
                       <div><select data-type="itemid" name="itemid[]"  id ="itemid_{{$i}}" class="form-control chosen-select" onchange="loadItemsDet(this.id,this.value)">
                           ...
                           @foreach($item_list as $cmb)
                               <option  {{ $cmb->id == $d->pur_item_id ? 'selected' : '' }} value="{{ $cmb->id }}">{{ $cmb->item_name }}({{ $cmb->itm_cat_name }})</option>
                           @endforeach
                         </select></div>
                      </td>
                       ...
                       <td width="5%" style="display: none" align="center">
                         <div><select data-type="Storage" name="Storage[]"  id ="Storage_{{$i}}" class="form-control chosen-select">
                             @foreach($stor_list as $stor)
                                 <option  {{ $stor->id == $d->pur_lot_no ? 'selected' : '' }} value="{{ $stor->id }}">{{ $stor->stor_code }}({{ $stor->stor_name }})</option>
                             @endforeach
                           </select></div>
                       </td>
                       <td width="4%" align="center"><input type="text" data-type="Unit" name="Unit[]" id="Unit_{{$i}}" value="{{$d->pur_item_unit}}" class="form-control input-sm"  autocomplete="off" readonly></td>
                       <td width="12%" align="center"> <input type="text" size = "8" name="exp_date_{{$i}}" id="exp_date_{{$i}}" onclick="displayDatePicker('exp_date_{{$i}}');"  value={{ date('d-m-Y',strtotime($d->pur_item_exp_dt)) }} />
                       ...
                       <td width="8%" align="center"><input type="text" data-type="Stock" name="Stock[]" id="Stock_{{$i}}" value="{{$d->po_item_qty-$d->po_item_qty_rec+$d->pur_item_qty}}" class="form-control input-sm" style="font-weight:bold; text-align: center;" autocomplete="off" readonly></td>
                       <td width="15%" align="center"><input type="text" data-type="LotNo" name="LotNo[]" id="LotNo_{{$i}}" value="{{$d->pur_lot_no}}" class="form-control input-sm" style="font-weight:bold; text-align: center;" autocomplete="off" ></td>
                       <td width="7%" style="display: none" align="center"><input type="text" data-type="OldQty" name="OldQty[]" id="OldQty_{{$i}}" value="{{$d->pur_item_qty}}" class="form-control input-sm" style="font-weight:bold; text-align: center;" autocomplete="off" ></td>
                       <td width="7%" align="center"><input type="text" data-type="Qty" name="Qty[]" id="Qty_{{$i}}" value="{{$d->pur_item_qty}}" onkeydown="enter(this.id,this.value)" class="form-control input-sm changesNo iQty" style="font-weight:bold; text-align: center;" autocomplete="off" ></td>
                       <td width="6%" align="right"><input type="text" data-type="Rate" name="Rate[]" id="Rate_{{$i}}" value="{{$d->pur_item_price}}" onkeydown="enter(this.id,this.value)" class="form-control input-sm changesNo iRate" style="font-weight:bold; text-align: center;" autocomplete="off"></td>
                       <td width="8%" align="right"><input type="text" data-type="Amount" name="Amount[]" id="Amount_{{$i}}" value="{{$d->pur_item_qty * $d->pur_item_price}}" class="form-control input-sm iAmount" style="font-weight:bold; text-align: right;" autocomplete="off" readonly></td>
                       <td width="8%" align="right"><input type="text" data-type="AmountBDT" name="AmountBDT[]" id="AmountBDT_{{$i}}" value="{{$d->pur_item_qty * $d->pur_item_price*$d->pur_d_item_curr_rate}}"class="form-control input-sm iAmountBDT" style="font-weight:bold; text-align: right;" autocomplete="off" readonly></td>
                       <td width="15%" align="right"><input type="text" data-type="Remarks" name="Remarks[]" id="Remarks_{{$i}}" value="{{$d->pur_item_remarks}}" class="form-control input-sm" style="font-weight:bold; text-align: left;" autocomplete="off" ></td>
                 ...
                 <table class="table table-striped table-data table-view">
                   <tbody style="background-color: #ffffff;">
                     <tr>
                       ...
                       <td width="2%" align="center"><input type="text" data-type="total_qty" name="total_qty" id="total_qty" value="{{$rows_m->pur_total_qty}}" style="font-weight:bold; text-align: right;" readonly></td>
                       <td width="2%" align="right"><input type="text" data-type="total_amount" name="total_amount" id="total_amount" value="{{$rows_m->pur_total_amount}}" style="font-weight:bold; text-align: right;" readonly></td>
                       <td width="2%" align="right"><input type="text" data-type="total_amount" name="total_amount_bdt" id="total_amount_bdt" value="{{$rows_m->pur_total_amount_bdt}}" style="font-weight:bold; text-align: right;" readonly></td>
                       <td width="3%"></td>
                     </tr>
                    </tbody>
                 </table>
...
function loadItemsDet(el,itemid){
  var purchase_no = $('#res_purchase_no').val();
