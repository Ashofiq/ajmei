...
<section class="content">
  ...
  <div  style="background-color:#e0e0e0" class="widget-header widget-header-small">
    ...
    <font size="2" color="blue"><b>Edit:Inventory Transfer Form</b></font>
    </h6>
    <div class="widget-toolbar">
      <a href="{{route('itm.inv.transfer.index')}}" class="blue"><i class="fa fa-list"></i> List</a>
    </div>
  </div>
</div>
...
@endif
  <form id="inv_Form" action="{{route('itm.inv.transfer.update')}}" method="post">
    {{ csrf_field() }}
         ...
         <div class="row">
          <div class="col-md-3">
            <div class="input-group ss-item-required">
                ...
                <input type="text" size = "15" name="trans_date" onclick="displayDatePicker('trans_date');"  value={{ date('d-m-Y',strtotime($rows_m->trans_date)) }} />
        ...
        <div class="row">
            <div class="col-md-10">
                <div class="input-group ss-item-required">
                    ...
                    <textarea name="comments" rows="2" cols="100" class="form-control config" maxlength="500" required>{{ $rows_m->trans_comments }}</textarea>
                </div>
             </div>
        </div>
        <div class="row">
                 ...
                 <option value="" >--Select Wearhouse--</option>
                     @if ($warehouse_list->count())
                         @foreach($warehouse_list as $list)
                             <option {{ $rows_m->trans_m_sou_ware_id == $list->w_ref_id ? 'selected' : '' }} value="{{$list->w_ref_id}}" >{{ $list->ware_name }}</option>
                         @endforeach
                     @endif
                 </select>
                </div>
           ...
           <div class="col-md-4">
              <div class="input-group-prepend ss-item-required">
                ...
                <select name="itm_rec_warehouse" class="chosen-select" id="itm_rec_warehouse" required>
                <option value="" >--Select Wearhouse--</option>
                    @if ($warehouse_list->count())
                        @foreach($warehouse_list as $list)
                            <option {{ $rows_m->trans_m_rec_ware_id  == $list->w_ref_id ? 'selected' : '' }} value="{{$list->w_ref_id}}" >{{ $list->ware_name }}</option>
                        @endforeach
                    @endif
                </select>
               </div>
           ...
           <div class="col-md-3">
             <div class="input-group ss-item-required">
               <input type="text" name="result_wh_id" id="result_wh_id" value="{{$rows_m->trans_m_sou_ware_id}}" class="form-control" readonly required/>
               <input type="text" name="result_storage_id" id="result_storage_id" value="{{$rows_m->trans_m_sou_ware_id}}" class="form-control" readonly required/>
             </div>
           </div>
         </div>
       <div class="row">
                    ...
                    @foreach($rows_d as $d)
                       ...
                       <div><select data-type="itemid" name="itemid[]"  id ="itemid_{{$i}}" class="form-control chosen-select" onchange="loadItemsDet(this.id,this.value)">
                           ...
                           @foreach($item_list as $cmb)
                               <option  {{ $cmb->id == $d->trans_item_id ? 'selected' : '' }} value="{{ $cmb->id }}">{{ $cmb->item_name }}({{ $cmb->itm_cat_name }})</option>
                           @endforeach
                         </select></div>
                      </td>
                        ...
                        <td width="5%" style="display: none" align="center">
                         <div><select data-type="Storage" name="Storage[]"  id ="Storage_{{$i}}" class="form-control chosen-select">
                             @foreach($stor_list as $stor)
                                 <option {{ $stor->id == $d->trans_storage_id ? 'selected' : '' }} value="{{ $stor->id }}">{{ $stor->stor_code }}({{ $stor->stor_name }})</option>
                             @endforeach
                           </select></div>
                       </td>
                       <td width="10%">
                         <div><select data-type="lotno" name="lotno[]"  id ="lotno_{{$i}}" class="form-control chosen-select" onchange="loadLotDet(this.id,this.value)">
                           <option value="{{ $d->trans_lot_no }}">{{$d->trans_lot_no}}</option>
                         </select></div>
                       </td>
                       ...
                       <td width="8%" align="center" ><input type="text" data-type="Stock" name="Stock[]" id="Stock_{{$i}}"  value="{{$d->item_bal_stock + $d->trans_item_qty}}" class="form-control input-sm" style="font-weight:bold; text-align: center;" autocomplete="off" readonly></td>
                       ...
                       <td width="6%" align="center"><input type="text" data-type="Qty" name="Qty[]" id="Qty_{{$i}}" value="{{$d->trans_item_qty}}" onkeydown="enter(this.id,this.value)" class="form-control input-sm changesNo iQty" style="font-weight:bold; text-align: center;" autocomplete="off" ></td>
