...
<div class="title">
  <div  style="background-color:#e0e0e0" class="widget-header widget-header-small">
    ...
    <font size="2" color="blue"><b>Inventory Transfer Form</b></font>
    </h6>
...
@endif
  <form id="inv_Form" action="{{route('itm.inv.transfer.store')}}" method="post">
    {{ csrf_field() }}
         ...
         <div class="row">
          <div class="col-md-3">
            <div class="input-group ss-item-required">
                ...
                <input type="text" size = "15" name="trans_date" onclick="displayDatePicker('trans_date');"  value={{ date('d-m-Y',strtotime($trans_date)) }} />
         ...
         <div class="col-md-4">
            <div class="input-group-prepend ss-item-required">
              ...
              <option value="" >--Select Wearhouse--</option>
                  @if ($warehouse_list->count())
                      @foreach($warehouse_list as $list)
                          <option {{ old('itm_rec_warehouse') == $list->w_ref_id ? 'selected' : '' }} value="{{$list->w_ref_id}}" >{{ $list->ware_name }}</option>
                      @endforeach
                  @endif
              </select>
             </div>
                     ...
                     <tr>
                       ...
                       <td width="10%">
                         <div><select data-type="lotno" name="lotno[]"  id ="lotno_1" class="form-control chosen-select" onchange="loadLotDet(this.id,this.value)">
                           <option value="" selected>- Item LOT1 -</option>
                         </select></div>
                       </td>
...
document.getElementById('btn2').onclick = function() {
  form.action = '{{route("itm.inv.transfer.store")}}';
  if(formcheck()){
    form.submit();
  }
}
...
function formcheck() {
  ...
  var itm_rec_warehouse = $('#itm_rec_warehouse').val();
  var itm_warehouse = $('#itm_warehouse').val();
  if(itm_rec_warehouse == itm_warehouse){
    alert("Supplying & Receiving Warehouse can not Same");
    isSubmit = false;
    return isSubmit;
  }
  return isSubmit;
