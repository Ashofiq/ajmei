...
<div class="title">
  <div  style="background-color:#e0e0e0" class="widget-header widget-header-small">
    ...
    <font size="2" color="blue"><b>Edit:Loan Receive From Customer</b></font>
    </h6>
...
@endif
  <form id="so_Form" action="{{route('sales.loan.issue.update')}}" method="post">
    {{ csrf_field() }}
    <input type="hidden" id="loan_id" name="loan_id" value="{{ $mas->id }}" class="form-control  input-sm" autocomplete="off" required/>
         ...
         <div class="row">
          <div class="col-md-3">
            <div class="input-group ss-item-required">
                ...
                <input type="text" size = "15" name="order_date" onclick="displayDatePicker('order_date');"  value="{{ date('d-m-Y',strtotime($mas->loan_i_order_date)) }}"  required />
          ...
          <div class="col-md-3">
            <div class="input-group ss-item-required">
                ...
                <input type="text" name="sales_orderno" value="{{ $mas->loan_i_order_no }}" class="form-control  input-sm" autocomplete="off" required readonly/>
           </div>
          </div>
          <div class="col-md-4">
                 ...
                 <option value="" >--Select--</option>
                     @if ($companies->count())
                         @foreach($companies as $company)
                             <option {{ $mas->loan_i_comp_id == $company->id ? 'selected' : '' }} value="{{$company->id}}" >{{ $company->id }}-{{ $company->name }}</option>
                         @endforeach
                     @endif
                 </select>
                </div>
      ...
      <div class="row">
        <div class="col-md-3">
          <div class="input-group">
            ...
            <input type="text" name="reference_no" id="reference_no" value="{{$mas->loan_i_reference}}" class="form-control" required/>
          </div>
        </div>
            ...
            <div class="input-group">
              <select name="customer_id" id="customer_id" class="chosen-select"  onchange="getCustomerDetails(this.value)">
                  ...
                  @foreach($customers as $customer)
                      <option {{ $mas->loan_i_cust_id == $customer->id ? 'selected' : '' }} value="{{ $customer->id }}">{{ $customer->cust_name }}</option>
                  @endforeach
              </select>
              @error('customer_id')
       ...
       <div class="row">
                ...
                <select name="itm_warehouse" class="chosen-select" id="itm_warehouse" onchange="getStorageByWearId(this.value)" required>
                  ...
                  @if ($warehouse_list->count())
                        @foreach($warehouse_list as $list)
                            <option {{ $mas->loan_i_m_warehouse_id == $list->w_ref_id ? 'selected' : '' }} value="{{$list->w_ref_id}}" >{{ $list->ware_name }}</option>
                        @endforeach
                  @endif
                </select>
               </div>
          ...
          <div class="col-md-4">
            <div class="input-group ss-item-required">
              <input type="hidden" name="result_wh_id" id="result_wh_id" value="{{ $mas->loan_i_m_warehouse_id }}" class="form-control" readonly required/>
              <input type="hidden" name="result_storage_id" id="result_storage_id" value="{{$mas->loan_i_m_warehouse_id}}" class="form-control" readonly required/> 
            </div>
          </div>
        </div><br/>
               ...
               <div role="tabpanel" class="tab-pane active" id = "itemdetails">
                 <div class="row">
                   <div class="col-md-12 input-group">
                     <table id="salesTable" class="table table-striped table-data table-view">
                       <thead class="salesTable">
                         ...
                         <th width="10%" style="display: none" class="text-center">Item Desc</th>
                   ...
                   <tbody class="salesTable" style="background-color: #ffffff;">
                    ...
                    @foreach($det as $d)
                    <?php
                      $s_total_amt  = $d->loan_i_item_price*$d->loan_i_qty;
                    ...
                    ?>
                     <tr>
                       ...
                       <td width="10%" style="display: none"><input type="text" data-type="ItemDesc" name="ItemDesc[]" id="ItemDesc_{{$i}}" value="{{$d->item_desc}}" class="form-control" autocomplete="off" ></td>
                       ...
                       <td width="5%" style="display: none" align="center">
                         <div><select data-type="Storage" name="Storage[]"  id ="Storage_{{$i}}" class="form-control chosen-select">
                             @foreach($stor_list as $stor)
                                 <option {{ $stor->id == $d->loan_i_storage_id ? 'selected' : '' }} value="{{ $stor->id }}">{{ $stor->stor_code }}({{ $stor->stor_name }})</option>
                             @endforeach
                           </select></div>
                       </td>

                       <td width="10%">
                         <div><input type="text" data-type="lotno" name="lotno[]" id="lotno_{{$i}}"  value="{{$d->loan_i_lot_no}}" class="form-control input-sm" style="font-weight:bold; text-align: left;" autocomplete="off"></div>
                       </td> 
 
                       <td width="5%" align="center"><input type="text" data-type="Qty" name="Qty[]" id="Qty_{{$i}}" value="{{$d->loan_i_qty}}" onkeydown="enter(this.id,this.value)" class="form-control input-sm changesNo iQty" style="font-weight:bold; text-align: center;" autocomplete="off" ></td>
                 ...
                 <table class="table table-striped table-data table-view">
                   <tbody style="background-color: #ffffff;"> 
                    <tr>
                       ...
                       <td width="8%"></td>
                       <td width="15%"></td> 
                       <td width="15%" align="right"><b>Total Qty:<b/></td>
...
<script>
  ...
  document.getElementById('btn1').onclick = function() {
    form.action = '{{route("sales.loan.issue.update")}}';
    if(formcheck()){
      form.submit();
    }
  }
