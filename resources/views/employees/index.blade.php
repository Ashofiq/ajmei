...
<div class="title">
  <div class="widget-header widget-header-small">
    <font size="3" color="blue"><b>Employee Information</b></font>
      ...
      <div class="col-md-5">
        <div class="input-group ss-item-required"> 
          <select name="employee_id" class="col-xs-6 col-sm-4 chosen-select" id="employee_id" required>
              <option value="" disabled selected>- Search ID -</option>
                @foreach($employees as $employee)
                  <option {{ old('employee_id') == $employee->id ? 'selected' : '' }} value="{{ $employee->id }}">{{ $employee->emp_name }} -- {{ $employee->emp_id_no  }}</option>
                @endforeach
           </select>
            @error('employee_id')
      ...
      <table class="table table-striped table-view">
        ...
        <tbody>
          ...
          <tr>
            ...
            <td width="10%">{{ $row->emp_id_no }}</td>
            <td width="15%">{{ $row->emp_name }}</td>
            <td width="16%">{{ $row->designation }}</td>
            <td width="8%">{{ $row->department }}</td> 
            <td width="8%">{{ $row->section }}</td> 
            <td width="20%">{{ date('d-m-Y',strtotime($row->emp_joining_dt)) }}</td> 
            <td width="18%">
              <form  method="post" action="{{ url('/employees/destroy/'.$row->id) }}" class="delete_form">
                {{ csrf_field() }}
                {{ method_field('DELETE') }} 
                <div class="btn-group btn-corner">
                  <a><a href="{{ route('employees.edit',$row->id) }}" class="btn btn-xs btn-primary edit">Edit</a>
                  <span href="{{ url('/') }}/employees-m-view/{{$row->id}}" data-toggle="modal" 
                  data-id="{{$row->id}}" class="btn btn-sm btn-info viewModal" title="View Details" data-placement="top" >View</span>  
