...
<div class="title">
    <div  style="background-color:#e0e0e0" class="widget-header widget-header-small">
        ...
        <font size="2" color="blue"><b>Employee Entry</b></font>
        </h6>
...
@endif
    ...
    <div class="widget-body">
      <div class="widget-main">  
       <form id="emp_Form" action="{{route('employees.store')}}" method="post">  
       {{ csrf_field() }}
       <div class="row">
          <div class="col-md-4">
          ...
          <div class="col-md-3">
              <div class="input-group">
                  <div class="input-group-prepend">
                      ...
                      <input type="text" name="uid_no_old" class="form-control" value="{{old('uid_no_old')}}"/>
                  </div>
             </div>
          </div>
          <div class="col-md-4">
                   ...
                   @if ($sysinfos->count())
                       @foreach($sysinfos as $d)
                            @if($d->vComboType == 'Department')
                                <option {{ old('department') == $d->id ? 'selected' : '' }} value="{{ $d->id  }}" >{{ $d->vComboName }}</option>
                           @endif
                       @endforeach
                   @endif
               </select>
                            ...
                            @if ($sysinfos->count())
                                @foreach($sysinfos as $d)
                                    @if($d->vComboType == 'Section')
                                        <option {{ old('section') == $d->id ? 'selected' : '' }} value="{{ $d->id  }}" >{{ $d->vComboName }}</option>
                                    @endif
                                @endforeach
                            @endif
                    </select> 
        ...
        <div class="row">
            ...
            <!-- <div class="col-md-4">
                <div class="input-group">
                    <div class="input-group-prepend">
                        <div class="input-group-text" style="min-width:130px">Skill Grade:</div>
                    </div>
                    <input type="text" name="skill_grade" class="form-control" value="{{old('skill_grade')}}"/>
               </div>
            </div> -->
        </div> 
        <div class="row">
                   ...
                   @if ($sysinfos->count())
                       @foreach($sysinfos as $d)
                            @if($d->vComboType == 'Unit')
                                <option {{ old('section_category') == $d->id ? 'selected' : '' }} value="{{ $d->id  }}" >{{ $d->vComboName }}</option>
                            @endif
                       @endforeach
                   @endif
               </select>
            ...
            <div class="col-md-0">
                <div class="input-group">  
                    ...
                    data-target-id="Unit" data-target="#addModal">New Unit</button>
               </div>
            </div> 
        </div> 
        <!-- <div class="row">
            <div class="col-md-6">
             <div class="input-group-prepend ss-item-required">
               <div class="input-group-prepend">
                   <div class="input-group-text" style="min-width:130px">Type:</div>
               </div>
               <select name="type" class="chosen-select" id="Type">
               <option value="" >--Select Type--</option>y
                   @if ($sysinfos->count())
                       @foreach($sysinfos as $d)
                            @if($d->vComboType == 'Type')
                                <option {{ old('type') == $d->id ? 'selected' : '' }} value="{{ $d->id  }}" >{{ $d->vComboName }}</option>
                            @endif
                       @endforeach
                   @endif
               </select>
            ...
            <div class="col-md-0">
                <div class="input-group">  
                    <button type="button" class="btn btn-sm btn-success" data-toggle="modal" data-target-id="Type" data-target="#addModal">New Type</button>
               </div>
            </div> 
        </div>  -->
        <div class="row">
                   ...
                   @if ($sysinfos->count())
                       @foreach($sysinfos as $d)
                            @if($d->vComboType == 'Shift')
                                <option {{ old('shift') == $d->id ? 'selected' : '' }} value="{{ $d->id  }}" >{{ $d->vComboName }}</option>
                            @endif 
                       @endforeach
                   @endif
               </select>
        ...
        <div class="row">
          <div class="col-md-3">
            <div class="input-group ss-item-required">
                ...
                <input type="text" size = "10" name="joining_date" onclick="displayDatePicker('joining_date');"  value="{{ old('joining_date') == '' ?  date('d-m-Y') :  date('d-m-Y',strtotime(old('joining_date'))) }}"  required />
          ...
          <div class="col-md-3">
            <div class="input-group ss-item-required">
                ...
                <input type="text" size = "10" name="birth_date" onclick="displayDatePicker('birth_date');"  value="{{ old('birth_date') == '' ?  date('d-m-Y') :  date('d-m-Y',strtotime(old('birth_date'))) }}" />
        ...
        <div class="row">
            ...
            <div class="input-group">
                ...
                <input type="text" name="joining_salary" id="joining_salary" value="{{old('joining_salary')}}" required class="form-control"/>
            </div>
            </div>
            <div class="col-md-3">
            <div class="input-group">
                ...
                <input type="text" name="present_salary" id="present_salary" value="{{old('present_salary')}}" class="form-control"/>
            </div>
            </div>
        </div>
        ...
        <!-- <div class="row">
            <div class="col-md-3">
                <div class="input-group">
                    ...
                    <input type="text" name="salary_grade" id="salary_grade" value="{{old('salary_grade')}}" required class="form-control"/>
                </div>
            </div>
            <div class="col-md-3">
                <div class="input-group">
                    ...
                    <input type="text" name="actual_salary" id="actual_salary" value="{{old('actual_salary')}}" class="form-control"/>
                </div>
            </div>
            <div class="col-md-3">
                <div class="input-group">
                    ...
                    <input type="text" name="others_salary" id="others_salary" value="{{old('others_salary')}}" class="form-control"/>
                </div>
            </div>
        </div> -->
        ...
        <div class="row">
            <div class="col-md-3">
             <div class="input-group-prepend ss-item-required">
               ...
               <select name="pay_type" class="chosen-select" id="pay_type" required>
                ...
                <!-- <option value="" >--Select Pay Type--</option>y
                    @if ($sysinfos->count())  
                        @foreach($sysinfos as $d)
                                @if($d->vComboType == 'Pay Type')
                                    <option {{ old('pay_type') == $d->id ? 'selected' : '' }} value="{{ $d->id  }}" >{{ $d->vComboName }}</option>
                                @endif  
                        @endforeach
                    @endif
                </select> -->
            ...
            <div class="col-md-3">
             <div class="input-group-prepend ss-item-required">
               <div class="input-group-prepend">
                   <div class="input-group-text" style="min-width:130px">Category:</div>
               </div>
               <select name="category1" class="chosen-select" id="category1" >
                   @if ($sysinfos->count())
                       @foreach($sysinfos as $d)
                            @if($d->vComboType == 'Category1')
                                <option {{ old('category1') == $d->id ? 'selected' : '' }} value="{{ $d->id  }}" >{{ $d->vComboName }}</option>
                            @endif  
                       @endforeach
                   @endif
               </select>
            ...
            <div class="col-md-3">
             <div class="input-group-prepend ss-item-required">
               ...
               <select name="ot_type" class="chosen-select" id="ot_type" required>
                    <option value="Y">Y</option>
                    <option value="N">N</option>
                   @if ($sysinfos->count())
                       @foreach($sysinfos as $d)
                            @if($d->vComboType == 'OT type')
                                <option {{ old('ot_type') == $d->id ? 'selected' : '' }} value="{{ $d->id  }}" >{{ $d->vComboName }}</option>
                            @endif  
                       @endforeach
                   @endif
               </select>
        ...
        <div class="row">
            ...
            <div class="col-md-3">
                <div class="input-group ss-item-required">
                    ...
                    value="{{ old('out_date') == '' ?  '' :  date('d-m-Y',strtotime(old('out_date'))) }}" />
        ...
        <div class="row">
            ...
            <div class="col-md-12">
             <div class="input-group-prepend ss-item-required">
               ...
               <textarea name="remark" style="width: 416px"></textarea>
              </div>
            </div> 
         </div> 
            ...
            jQuery('#ajaxSubmit').click(function(e){
               ...
               jQuery.ajax({
                  ...
                  data: {
                    ...
                    attn_bonus: jQuery('#attn_bonus').val()
                  },
