...
<div class="title">
  <div  style="background-color:#e0e0e0" class="widget-header widget-header-small">
    ...
    <font size="2" color="blue"><b>Issue to Production Form</b></font>
    </h6>
...
@endif
  <form id="inv_Form" action="{{route('raw.itm.issue.store')}}" method="post">
    {{ csrf_field() }}
                 ...
                 <div class="row">
                   ...
                   <tbody class="salesTable" style="background-color: #ffffff;">
                     <tr>
                       ...
                       <td width="6%" align="right"><input type="text" onkeyup="quantityCng(this.value, this.id)" data-type="Rate" name="Rate[]" id="Rate_1" Value="0.00" onkeydown="enter(this.id,this.value)" class="form-control input-sm changesNo iRate" style="font-weight:bold; text-align: center;" autocomplete="off"></td>
...
$(document).on('change keyup blur', '.changesNo', function () {
  ...
  id = id_arr.split("_");
  //alert(id[0]);
  currencyValue = $('#currencyValue').val().replace(/,/g, ''); 
  price         = $('#Rate_' + id[1]).val().replace(/,/g, '');
  quantity      = $('#Qty_' + id[1]).val().replace(/,/g, '');

  if (quantity != '' && price != ''){
    var formatter = new Intl.NumberFormat('en-US', {currency: 'BDT'});

      amount = (parseFloat(price)*parseFloat(quantity)).toFixed(2);
      $('#Amount_' + id[1]).val(formatter.format(amount));
      bdamount = (parseFloat(amount) * parseFloat(currencyValue)).toFixed(2);
      $('#AmountBDT_' + id[1]).val(formatter.format(bdamount));
  }else{
    $('#Amount_' + id[1]).val(0);
...
function totalQuantityCount()
{
    ...
    $('.iQty').each(function(){
        if(parseFloat($(this).val())>0)
            total_qty += parseFloat($(this).val().replace(/,/g, ''));
    });
...
function totalAmountCount()
{
  var formatter = new Intl.NumberFormat('en-US', {currency: 'BDT'});
    ...
    $('#total_amount').val(formatter.format(parseFloat(total_amount).toFixed(2)));
}
