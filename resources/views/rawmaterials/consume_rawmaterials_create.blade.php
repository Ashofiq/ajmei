...
<div class="title">
  <div  style="background-color:#e0e0e0" class="widget-header widget-header-small">
    ...
    <font size="2" color="blue"><b>Item Consumed Form </b></font>
    </h6>
...
@endif
  <form id="inv_Form" action="{{route('itm.consumable.store')}}" method="post">
    @csrf
    <div class="widget-body">
         ...
         <div class="row"> 
          <div class="col-md-3">
            <div class="input-group ss-item-required">
                ...
                <input type="text" size = "15" name="issue_date" onclick="displayDatePicker('issue_date');"  value={{ date('d-m-Y',strtotime($issue_date)) }} />
      ...
      <div class="row">
        <div class="col-md-3">
          <div class="input-group ss-item-required">
            <div class="input-group-prepend">
              <div class="input-group-text" style="min-width:80px">Issue Invoice No:</div>
            </div>
        ...
        <div class="row">
          <div class="col-md-12">
              <div class="input-group">
                  ...
                  <textarea class="" rows="2" cols="100" name="comments" id="comments">{{ old('comments') }}</textarea>
              </div>
           </div>
        </div>
        ...
        <!-- <div class="row">
          <div class="col-md-12">
            <div class="input-group">
              <div class="input-group-prepend">
                  <div class="input-group-text" style="min-width:80px">Debit Head:</div>
              </div>
              <select data-type="debitHead" name="debitHead"  id ="debitHead" class="form-control chosen-select" >
                <option value="" disabled selected>- Select Item -</option>
                @foreach($debitHead as $row)
                    <option  value="{{ $row->id }}">{{ $row->acc_head }} >> {{ $row->acc_origin }}</option>
                @endforeach
              </select>
            </div>
           </div>
                 ...
                 <div class="row">
                   ...
                   <tbody class="salesTable" style="background-color: #ffffff;">
                     <tr>
                       ...
                       <td width="7%" style="display: none"><input type="text" data-type="ItemBarCode" name="ItemBarCode[]" id="ItemBarCode_1" onKeyUp="loadItemsDetByBarcode(this.id,this.value)" class="form-control autocomplete_barcode_txt" autocomplete="off"></td>
                       <td width="23%">
                       <div><select data-type="itemid" name="itemid[]"  id ="itemid_1" class="form-control chosen-select" onchange="loadItemsDet(this.id,this.value)">
                           ...
                           @foreach($item_list as $cmb)
                               <option  value="{{ $cmb->id }}">{{ $cmb->item_name }}({{ $cmb->itm_cat_origin }} {{ $cmb->itm_cat_name }})</option>
                           @endforeach
                         </select></div>
                      </td>
                       <td width="23%"><input type="text" data-type="ItemDesc" onkeydown="enter(this.id,this.value)" name="ItemDesc[]" id="ItemDesc_1" class="form-control" autocomplete="off" ></td>
                       ...
                       <td width="8%"><div><select data-type="Unit" name="Unit[]"  id ="Unit_1" class="form-control chosen-select">
                           <option value="" disabled selected>- Select Unit -</option>
                           @foreach($unit_list as $cmb)
                               <option  value="{{ $cmb->vUnitName }}">{{ $cmb->vUnitName }}</option>
                           @endforeach
                         </select></div>
                         <!-- <input type="text" data-type="Unit" name="Unit[]" id="Unit_1"  class="form-control input-sm"  autocomplete="off" readonly> -->
                        </td>
                       <!--td width="10%" align="center"> <input type="text" size = "10" name="exp_date_1" id="exp_date_1" onclick="displayDatePicker('exp_date_1');"  value={{ date('d-m-Y',strtotime($issue_date )) }} />
                       ...
                       <td width="6%" align="center"><input type="text" onkeyup="quantityCng(this.value, this.id)" data-type="Qty" name="Qty[]" id="Qty_1" onkeydown="enter(this.id,this.value)" class="form-control input-sm changesNo iQty" style="font-weight:bold; text-align: center;" autocomplete="off" ></td>
                       <td width="6%" align="right"><input type="text" data-type="Rate" name="Rate[]" id="Rate_1" Value="1.00" onkeydown="enter(this.id,this.value)" class="form-control input-sm changesNo iRate" style="font-weight:bold; text-align: center;" autocomplete="off"></td>
                       ...
                       <td width="8%" align="right" style="display: none"><input type="text" data-type="AmountBDT" name="AmountBDT[]" id="AmountBDT_1" class="form-control input-sm iAmountBDT" style="font-weight:bold; text-align: right;" autocomplete="off" readonly></td>
                       <td width="15%" align="right"><input type="text" data-type="Remarks" onkeydown="enter(this.id,this.value)" name="Remarks[]" id="Remarks_1" class="form-control input-sm" style="font-weight:bold; text-align: left;" autocomplete="off" ></td>
                       <td width="3.5%"></td>
                   </tr>
                    </tbody>
                 </table>
      ...
      <div class="row justify-content-left">
        <div class="col-sm-12 text-left">
            <button class="btn btn-sm btn-success"  type="submit" id='btn1' ><i class="fa fa-save"></i> Save</button> 
...
function quantityCng(number, id) {
    // number = parseFloat(number.replace(/,/g, ''))

    // var formatter = new Intl.NumberFormat('en-US', {currency: 'BDT'});
    ...
    var possition = id.split("_")[1];
    var stock = parseFloat($('#Stock_'+possition).val());
...
//   form.action = '{{route("itm.consumable.store")}}';
//   if(formcheck()){
//     form.submit();
//     return false;
//   }
...
function loadItemsDet(el,itemid){
    ...
    item = data.data
      ...
      console.log(item);
      $('#Rate_'+id[1]).val(item.averagePrice.toFixed(2));
...
function row_increment() {
    ...
    html += '<td width="6%"><input type="text" data-type="Qty" onkeyup="quantityCng(this.value, this.id)" name="Qty[]" id="Qty_' + i + '"  onkeydown="enter(this.id,this.value)" class="form-control input-sm changesNo iQty" style="font-weight:bold; text-align: center;" autocomplete="off"></td>';
...
$(document).on('keypress', '.autocomplete_txt', function () {
 ...
 function checkDuplicateItem(id, names,s_tag){
    ...
    //  alert(i + ","+ compcode + "," + custid);
      $.get('{{ url('/') }}/consitemLookup/'+compcode, function(response) {
        ...
        $.each(response, function(index, element) {
          ...
          if (oldItem ==  element.id){
            selectList.append('<option value="' + element.id + '" selected>' + element.item_name +' ('+  element.itm_cat_origin + '' + element.itm_cat_name +')</option>');
          }else{
            selectList.append('<option value="' + element.id + '">' + element.item_name +' ('+  element.itm_cat_origin + '' + element.itm_cat_name +')</option>');
          }
        });
        selectList.trigger('chosen:updated');
      });
