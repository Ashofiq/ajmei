...
<section class="content">
  ...
  <div  style="background-color:#e0e0e0" class="widget-header widget-header-small">
    ...
    <font size="2" color="blue"><b>Edit: Issue to Production Form</b></font>
    </h6>
...
@endif
  <form id="inv_Form" action="{{route('raw.itm.issue.update')}}" method="post">
    {{ csrf_field() }}
         ...
         <div class="row"> 
          <div class="col-md-3">
            <div class="input-group ss-item-required">
                ...
                <input type="text" size = "15" name="issue_date" onclick="displayDatePicker('issue_date');"  value={{ date('d-m-Y',strtotime($rows_m->r_issue_order_date)) }} />
                 ...
                 <option value="" >--Select--</option>
                     @if ($companies->count())
                         @foreach($companies as $company)
                             <option {{ $rows_m->r_issue_comp_id == $company->comp_id ? 'selected' : '' }} value="{{$company->comp_id}}" >{{ $company->comp_id }}-{{ $company->name }}</option>
                         @endforeach
                     @endif
                 </select>
                </div>
           </div> 
            <input type="hidden" name="currencyValue" id="currencyValue" value="{{$rows_m->r_issue_m_curr_rate}}" class="form-control changesCurrency" required/>
            <input type="hidden" name="currencyName" id="currencyName" value="{{$rows_m->r_issue_m_curr}}" class="form-control" required readonly/>
      ...
      <div class="row">
        <div class="col-md-3">
          <div class="input-group ss-item-required">
            ...
            <input type="text" name="purchase_no" id="purchase_no" value="{{$rows_m->r_issue_order_ref}}" class="form-control" required readonly/>
          </div>
        </div> 
      </div>
        ...
        <div class="row">
         <div class="col-md-12">
             <div class="input-group ss-item-required">
                 ...
                 <textarea class="summernote" rows="100" cols="100" name="comments" id="comments">{!! $rows_m->r_issue_comments !!}</textarea>
                 <input type="text" name="result_storage_id" id="result_storage_id" value="1" class="form-control" readonly required/>
             </div>
          </div>
       </div>
                 ...
                 <div class="row">
                   ...
                   <tbody class="salesTable" style="background-color: #ffffff;">
                    ...
                    @foreach($rows_d as $d)
                       ...
                       <td width="2%" style="display: none"><input type="text" data-type="ItemCodeId" name="ItemCodeId[]" id="ItemCodeId_{{$i}}" value="{{$d->r_issue_item_id}}" class="form-control item_id_class" autocomplete="off"></td>
                       ...
                       <div><select data-type="itemid" name="itemid[]"  id ="itemid_{{$i}}" class="form-control chosen-select" onchange="loadItemsDet(this.id,this.value)">
                           ...
                           @foreach($item_list as $cmb)
                               <option  {{ $cmb->id == $d->r_issue_item_id ? 'selected' : '' }} value="{{ $cmb->id }}">{{ $cmb->item_name }}({{ $cmb->itm_cat_name }})</option>
                           @endforeach
                         </select></div>
                      </td>
                       <td width="20%"><input type="text" data-type="ItemDesc" name="ItemDesc[]" id="ItemDesc_{{$i}}" value="{{$d->r_issue_item_desc}}"  class="form-control" autocomplete="off" ></td>
                       ...
                       <td width="8%" align="center"><div><select data-type="Unit" name="Unit[]"  id ="Unit_{{$i}}" class="form-control chosen-select">
                           ...
                           <option  {{ $cmb->vUnitName == $d->r_issue_item_unit ? 'selected' : '' }} value="{{ $cmb->vUnitName }}">{{ $cmb->vUnitName }}</option> 
                           @endforeach
                         </select></div></td>
                       ...
                       <td width="8%" align="center" ><input type="text" data-type="Stock" name="Stock[]" id="Stock_{{$i}}"  value="{{$d->item_bal_stock - $d->r_issue_item_qty }}" class="form-control input-sm" style="font-weight:bold; text-align: center;" autocomplete="off" readonly></td -->
                       ...
                       <td width="6%" align="center"><input type="text" data-type="Qty" name="Qty[]" id="Qty_{{$i}}" value="{{$d->r_issue_item_qty}}" onkeydown="enter(this.id,this.value)" class="form-control input-sm changesNo iQty" style="font-weight:bold; text-align: center;" autocomplete="off" ></td>
                       <td width="6%" align="right"><input type="text" data-type="Rate" name="Rate[]" id="Rate_{{$i}}" value="{{$d->r_issue_item_price}}" onkeydown="enter(this.id,this.value)" class="form-control input-sm changesNo iRate" style="font-weight:bold; text-align: center;" autocomplete="off"></td>
                       <td width="8%" align="right"><input type="text" data-type="Amount" name="Amount[]" id="Amount_{{$i}}" value="{{$d->r_issue_item_qty * $d->r_issue_item_price}}" class="form-control input-sm iAmount" style="font-weight:bold; text-align: right;" autocomplete="off" readonly></td> 
                       <td width="15%" align="right"><input type="text" data-type="Remarks" name="Remarks[]" id="Remarks_{{$i}}" value="{{$d->r_issue_item_remarks}}" class="form-control input-sm" style="font-weight:bold; text-align: left;" autocomplete="off" ></td>
                 ...
                 <table class="table table-striped table-data table-view">
                   <tbody style="background-color: #ffffff;">
                     <tr>
                       ...
                       <td width="2%" align="center"><input type="text" data-type="total_qty" name="total_qty" id="total_qty" value="{{$rows_m->r_issue_total_qty}}" style="font-weight:bold; text-align: right;" readonly></td>
                       <td width="2%" align="right"><input type="text" data-type="total_amount" name="total_amount" id="total_amount" value="{{$rows_m->r_issue_total_amount}}" style="font-weight:bold; text-align: right;" readonly></td> 
                      <td width="3%"></td>
                     </tr>
                    </tbody>
                 </table>
...
function loadItemsDet(el,itemid){
    ...
    $.get('{{ url('/') }}/get-raw-item-code/getdetails/'+itemid+'/getfirst', function(data){
    item = data.data
    }).then(function(){ 
      $('#Rate_'+i).val(item.averagePrice.toFixed(2));
