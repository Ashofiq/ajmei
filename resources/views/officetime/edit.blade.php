...
<div class="title">
    <div  style="background-color:#e0e0e0" class="widget-header widget-header-small">
        ...
        <font size="2" color="blue"><b>Edit::Office Timing Update</b></font>
        </h6>
...
@endif
    <div class="widget-body">
      <div class="widget-main">  
       <form id="emp_Form" action="{{route('timesheet.update')}}" method="post">  
       {{ csrf_field() }}
       <input type="text" class="form-control input-sm" name="otime_edit_id" value="{{ $row->id }}" readonly />
        <div class="row">
            <div class="col-md-6">
                        ...
                        <option value="" >--Select Shift--</option>
                            @if ($sysinfos->count())
                                @foreach($sysinfos as $combo)
                                    <option {{ $row->iShiftId  == $combo->id ? 'selected' : '' }} value="{{ $combo->id }}">{{ $combo->vComboName }}</option>
                                @endforeach
                            @endif
                    </select> 
                </div>
                    ...
                    <option value="" >--Select--</option>
                        @if ($companies->count())
                            @foreach($companies as $company)
                                <option {{ $row->iTime_Comp_id == $company->comp_id ? 'selected' : '' }} value="{{$company->comp_id}}" >{{ $company->comp_id }}-{{ $company->name }}</option>
                            @endforeach
                        @endif
                    </select>
                </div>
        ...
        <div class="row">
          <div class="col-md-4">
            <div class="input-group ss-item-required">
                ...
                <input type="time" id="stime" name="stime" placeholder="Time" value="{{date('H:i:s',strtotime($row->dtStart))}}" /> 
           </div>
          </div>
          <div class="col-md-4">
            <div class="input-group ss-item-required">
                ...
                <input type="time" id="etime" name="etime" placeholder="Time" value="{{date('H:i:s',strtotime($row->dtEnd))}}" />  
           </div>
          </div>
        </div>
        ...
        <div class="row">
          <div class="col-md-4">
            <div class="input-group ss-item-required">
                ...
                <input type="text" id="time" name="grtime" placeholder="Time" value="{{date('H:i:s',strtotime($row->dtGraceTime))}}" >  
           </div>
          </div> 
        </div>
        <br/>
